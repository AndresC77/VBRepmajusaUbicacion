...
Begin VB.Form frmCargaFamiliar 
   BackColor       =   &H00DDDDDD&
   ...
   ClientTop       =   330
   ClientWidth     =   8280
   Icon            =   "frmCargaFamiliar.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmCargaFamiliar.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmCargaFamiliar"
...
Public cod_empleado As String
Private Sub IniDato()
    Tipo = "Carga Familiar "
    Tipo2 = "Carga Familiar "
    Me.Caption = Tipo
End Sub
...
Dim i As Long
    ...
    strSql = " SELECT car_codigo,car_nombre,car_papel,car_fecha_nac, ' ' as edad, car_fechamod, car_usumod, '0' as modi " & _
             " FROM empleado_carga " & _
             " WHERE emp_codigo = '" & strEmpresa & "'"
    If chkFiltroNombre.Value = 1 Then
        strSql = strSql & " AND  car_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY car_nombre "
    clsCon_Def.Ejecutar strSql
    Set VSFG.DataSource = clsCon_Def.adorec_Def.DataSource
    
    VSFG.ColComboList(3) = "CONYUGE|HIJO|PADRE|MADRE"
    'calculo de esdad
    meses = DateDiff("m", VSFG.TextMatrix(i, 4), Date)
    'Mostramos el resultado
    For i = 1 To VSFG.Rows - 1
    VSFG.TextMatrix(i, 5) = Val(meses / 12) & " años y " & (meses Mod 12) & " meses"
Next i
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
           strSql = " UPDATE empleado_carga " & _
                 " SET car_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                 " car_papel='" & UCase(VSFG.TextMatrix(i, 3)) & "'," & _
                 " car_fecha_nac='" & VSFG.TextMatrix(i, 4) & "'," & _
                 " car_fechamod=CURRENT_TIMESTAMP," & _
                 " car_usumod='" & strUsuario & "' " & _
                 " WHERE car_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
                 " AND emp_codigo='" & strEmpresa & "'"
           clsCon_Def.Ejecutar strSql, "M"
        'insert
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
           strSql = " SELECT CONCAT('C',LPAD(COALESCE(MAX(RIGHT(car_codigo,LENGTH(car_codigo)-1)),0)+1,10,'0')) as num" & _
                     " FROM empleado_carga " & _
                     " WHERE emp_codigo='" & strEmpresa & "' " & _
                     " ORDER BY car_codigo"
           clsCon_Def.Ejecutar strSql
           VSFG.TextMatrix(i, 1) = clsCon_Def.adorec_Def("num")
            ...
            If VSFG.TextMatrix(i, 2) = "" Then
                ...
                control = 1
            Else
                strSql = " INSERT INTO empleado_carga(epl_codigo,car_codigo,emp_codigo,car_nombre," & _
                         " car_papel, car_fecha_nac, car_fechamod,car_usumod) " & _
                         " VALUES ('" & cod_empleado & "','" & VSFG.TextMatrix(i, 1) & "','" & strEmpresa & "','" & UCase(VSFG.TextMatrix(i, 2)) & "', " & _
