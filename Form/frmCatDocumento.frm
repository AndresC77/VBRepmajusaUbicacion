...
Begin VB.Form frmCatDocumento 
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmCatDocumento.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmCatDocumento"
...
End Sub
Private Sub Carga()
    strSql = " SELECT doc_codigo,doc_nombre,doc_fechamod, doc_usumod, '0' as modi  " & _
             " FROM documento " & _
             " WHERE doc_codigo LIKE '%'"
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  doc_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  doc_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY doc_nombre "
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            ...
            strSql = " UPDATE documento " & _
                 " SET doc_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                 " doc_fechamod=CURRENT_TIMESTAMP," & _
                 " doc_usumod='" & strUsuario & "' " & _
                 " WHERE doc_codigo='" & VSFG.TextMatrix(i, 1) & "'"
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                ...
                control = 1
            Else
                strSql = " SELECT doc_codigo" & _
                    " FROM documento " & _
                    " WHERE doc_codigo='" & VSFG.TextMatrix(i, 1) & "'"
                clsCon_Def.Ejecutar strSql
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    strSql = " INSERT INTO documento(doc_codigo,doc_nombre,doc_fechamod,doc_usumod) " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
             strSql = " SELECT count(his_p_codigo) as existe " & _
                    " FROM historial_p " & _
                    " WHERE doc_codigo = '" & VSFG.TextMatrix(i, 1) & "'"
            clsCon_Def.Ejecutar (strSql)
