...
Begin VB.Form frmColeccion 
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmColeccion.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmColeccion"
...
End Sub
Private Sub Carga()
   strSql = " SELECT clc_codigo,clc_nombre,clc_fechamod, clc_usumod, '0' as modi  " & _
            " FROM coleccion " & _
            " WHERE emp_codigo='" & strEmpresa & "' "
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND clc_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND clc_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY clc_nombre "
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
           strSql = " UPDATE coleccion " & _
                    " SET clc_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                    " clc_fechamod=CURRENT_TIMESTAMP,clc_usumod='" & strUsuario & "' " & _
                    " WHERE clc_codigo='" & VSFG.TextMatrix(i, 1) & "' AND emp_codigo='" & strEmpresa & "'"
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                ...
                control = 1
            Else
                strSql = " SELECT clc_codigo " & _
                    " FROM coleccion " & _
                    " WHERE clc_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                    " AND emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar strSql
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                   strSql = " INSERT INTO coleccion(clc_codigo,emp_codigo,clc_nombre,clc_fechamod,clc_usumod) " & _
