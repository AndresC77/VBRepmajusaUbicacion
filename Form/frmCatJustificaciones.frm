...
Begin VB.Form frmCatJustificaciones 
   BackColor       =   &H00DDDDDD&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tipo de Permisos"
   ClientHeight    =   6960
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8280
   Icon            =   "frmCatJustificaciones.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmCatJustificaciones.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmCatJustificaciones"
...
End Sub
Private Sub Carga()
    strSql = " SELECT tip_per_codigo,tip_per_nombre,tip_per_descripcion,tip_per_fechamod,tip_per_usumod, '0' as modi " & _
             " FROM tipo_permiso " & _
             " WHERE emp_codigo = '" & strEmpresa & "' "
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND tip_per_descripcion LIKE  '%" & txtCodigo.Text & "%' "
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND tip_per_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY tip_per_nombre "
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            If VSFG.TextMatrix(i, 2) = "" Then
                ...
                control = 1
            Else
                strSql = " UPDATE tipo_permiso " & _
                      " SET tip_per_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                      " tip_per_descripcion='" & UCase(VSFG.TextMatrix(i, 3)) & "'," & _
                      " tip_per_fechamod=CURRENT_TIMESTAMP," & _
                      " tip_per_usumod='" & strUsuario & "' " & _
                      " WHERE tip_per_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            Else
                Dim CoDiGos As Long
                strSql = " SELECT COALESCE(MAX(tip_per_codigo),0)+1 " & _
                        " FROM tipo_permiso " & _
                        " WHERE emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar strSql
                CoDiGos = 1
                If clsCon_Def.adorec_Def.RecordCount > 0 Then
                    CoDiGos = FormatoD0(clsCon_Def.adorec_Def(0))
                End If
                
                strSql = " SELECT tip_per_codigo " & _
                        " FROM tipo_permiso " & _
                        " WHERE tip_per_codigo='" & CoDiGos & "' " & _
                        " AND emp_codigo='" & strEmpresa & "' "
                clsCon_Def.Ejecutar strSql
                
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    strSql = " INSERT INTO tipo_permiso(tip_per_codigo,emp_codigo,tip_per_nombre,tip_per_descripcion,tip_per_fechamod,tip_per_usumod) " & _
                            " VALUES ('" & CoDiGos & "','" & strEmpresa & "','" & UCase(VSFG.TextMatrix(i, 2)) & "','" & UCase(VSFG.TextMatrix(i, 3)) & "', " & _
