...
Begin VB.Form frmResponsableFlujo 
   BackColor       =   &H00DDDDDD&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Responsables de Flujos"
   ClientHeight    =   6960
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8280
   Icon            =   "frmResponsableFlujo.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmResponsableFlujo.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmResponsableFlujo"
...
Private Sub IniDato()
    Tipo = "Responsable de Flujo"
    Tipo2 = "Responsable de Flujo"
    Me.Caption = Tipo
End Sub
...
End Sub
Private Sub Carga()
    strSql = " SELECT res_flu_codigo,res_flu_nombre,res_flu_email,usu_codigo,res_flu_fechamod, res_flu_usumod, '0' as modi " & _
             " FROM responsable_flujo " & _
             " WHERE emp_codigo LIKE '" & strEmpresa & "'"
    If chkFiltroNombre.Value = 1 Then
        strSql = strSql & " AND  res_flu_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY res_flu_nombre "
    clsCon_Def.Ejecutar strSql
    Set VSFG.DataSource = clsCon_Def.adorec_Def.DataSource
    ucrtVSFG.PonerNum
    
    strSql = " SELECT usu_codigo,CONCAT(usu_apellido,' ',usu_nombre) as usunombre " & _
             " FROM usuario " & _
             " ORDER BY CONCAT(usu_apellido,' ',usu_nombre)"
    clsCon_Def.Ejecutar strSql
    
    VSFG.ColComboList(4) = VSFG.BuildComboList(clsCon_Def.adorec_Def, "usu_codigo,*usunombre", "usu_codigo")
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            strSql = " UPDATE responsable_flujo " & _
                 " SET res_flu_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                 " res_flu_email='" & UCase(VSFG.TextMatrix(i, 3)) & "'," & _
                 " usu_codigo='" & UCase(VSFG.TextMatrix(i, 4)) & "'," & _
                 " res_flu_fechamod=CURRENT_TIMESTAMP," & _
                 " res_flu_usumod='" & strUsuario & "' " & _
                 " WHERE emp_codigo='" & strEmpresa & "'" & _
                 " AND res_flu_codigo='" & VSFG.TextMatrix(i, 1) & "'"
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 3) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta email", vbInformation, "Ingreso"
                control = 1
            ElseIf VSFG.TextMatrix(i, 4) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta usuario", vbInformation, "Ingreso"
                control = 1
            Else
                strSql = " SELECT res_flu_codigo" & _
                    " FROM responsable_flujo " & _
                    " WHERE emp_codigo='" & strEmpresa & "'" & _
                    " AND res_flu_codigo='" & VSFG.TextMatrix(i, 1) & "'"
                clsCon_Def.Ejecutar strSql
                ...
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    strSql = " INSERT INTO responsable_flujo(emp_codigo, res_flu_codigo, " & _
                             " res_flu_nombre, res_flu_email, usu_codigo, " & _
                             " res_flu_fechamod, res_flu_usumod) " & _
                             " VALUES ('" & strEmpresa & "','" & UCase(VSFG.TextMatrix(i, 1)) & "'," & _
                             " '" & UCase(VSFG.TextMatrix(i, 2)) & "', '" & UCase(VSFG.TextMatrix(i, 3)) & "', '" & UCase(VSFG.TextMatrix(i, 4)) & "', " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            ...
            strSql = " SELECT count(res_flu_codigo) as existe " & _
                    " FROM det_flujo " & _
                    " WHERE res_flu_codigo = '" & VSFG.TextMatrix(i, 1) & "' " & _
