...
Begin VB.Form frmDepartamentos 
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmDepartamentos.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
End Sub
Private Sub Carga()
    strSql = " SELECT dto_codigo,dto_nombre,are_codigo,dto_ctaconta,dto_descripcion,dto_fechamod,dto_usumod, '0' as modi " & _
             " FROM departamento " & _
             " WHERE emp_codigo = '" & strEmpresa & "'"
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  dto_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  dto_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY dto_codigo,dto_nombre "
    clsCon_Def.Ejecutar strSql
    Set VSFG.DataSource = clsCon_Def.adorec_Def.DataSource
    ucrtVSFG.PonerNum
    
    strSql = " Select are_codigo,are_nombre From area " & _
             " Where emp_codigo = '" & strEmpresa & "' " & _
             " Order By are_codigo,are_nombre "
    clsCon_Def.Ejecutar strSql
    VSFG.ColComboList(3) = VSFG.BuildComboList(clsCon_Def.adorec_Def, "are_codigo,*are_nombre", "are_codigo")
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            ...
            ElseIf VSFG.TextMatrix(i, 3) = "" Then
                MsgBox "No puede modificar " & Tipo2 & " falta área", vbInformation, "Modificación"
                control = 1
            Else
           strSql = " UPDATE departamento " & _
                 " SET dto_nombre='" & UCase(Trim(VSFG.TextMatrix(i, 2))) & "'," & _
                 " are_codigo='" & VSFG.TextMatrix(i, 3) & "'," & _
                 " dto_descripcion='" & UCase(Trim(VSFG.TextMatrix(i, 5))) & "'," & _
                 " dto_ctaconta='" & VSFG.TextMatrix(i, 4) & "'," & _
                 " dto_fechamod=CURRENT_TIMESTAMP," & _
                 " dto_usumod='" & strUsuario & "' " & _
                 " WHERE dto_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf Trim(VSFG.TextMatrix(i, 2)) = "" Then
                ...
                control = 1
            ElseIf VSFG.TextMatrix(i, 3) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta área", vbInformation, "Ingreso"
                control = 1
            Else
                strSql = " SELECT dto_codigo" & _
                    " FROM departamento " & _
                    " WHERE dto_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                    " AND emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar strSql
                ...
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    ...
                    strSql = " INSERT INTO departamento(dto_codigo,emp_codigo,are_codigo,dto_nombre,dto_ctaconta,dto_descripcion,dto_fechamod,dto_usumod) " & _
                            " VALUES ('" & UCase(Trim(VSFG.TextMatrix(i, 1))) & "','" & strEmpresa & "','" & VSFG.TextMatrix(i, 3) & "','" & UCase(Trim(VSFG.TextMatrix(i, 2))) & "', " & _
                            "'" & VSFG.TextMatrix(i, 4) & "','" & UCase(Trim(VSFG.TextMatrix(i, 5))) & "', " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            srtsql = " SELECT count(*) as existe " & _
                     " FROM pre_plan_inversion " & _
                     " WHERE dto_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                     " AND emp_codigo='" & strEmpresa & "'" & _
                     " AND are_codigo = '" & VSFG.TextMatrix(i, 3) & "'"
            clsCon_Def.Ejecutar (strSql)
