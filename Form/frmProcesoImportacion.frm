...
Begin VB.Form frmProcesoImportacion 
   BackColor       =   &H00DDDDDD&
   ...
   ClientTop       =   330
   ClientWidth     =   8280
   Icon            =   "frmProcesoImportacion.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmProcesoImportacion.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmProcesoImportacion"
...
Private Sub IniDato()
    Tipo = " Procesos de Importación "
    Tipo2 = " Procesos de Importación "
    Me.Caption = Tipo
End Sub
...
Private Sub Carga()
    strSql = " SELECT pro_imp_codigo,pro_imp_nombre,pro_imp_descripcion,pro_imp_orden," & _
             " pro_imp_fechamod, pro_imp_usumod, '0' as modi" & _
             " FROM  proceso_importacion" & _
             " WHERE pro_imp_codigo LIKE '%'"
    clsCon_Def.Ejecutar (strSql)
    
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  pro_imp_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  pro_imp_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY pro_imp_codigo "
    clsCon_Def.Ejecutar strSql
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        strSql = " UPDATE proceso_importacion" & _
                 " SET pro_imp_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "',pro_imp_descripcion='" & UCase(VSFG.TextMatrix(i, 3)) & "',pro_imp_orden=" & VSFG.TextMatrix(i, 4) & ", pro_imp_fechamod=CURRENT_TIMESTAMP,pro_imp_usumod='" & strUsuario & "' " & _
                 " WHERE pro_imp_codigo='" & VSFG.TextMatrix(i, 1) & "'"
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                ...
                control = 1
            Else
                strSql = " SELECT pro_imp_codigo" & _
                    " FROM proceso_importacion " & _
                    " WHERE pro_imp_codigo='" & VSFG.TextMatrix(i, 1) & "'"
                clsCon_Def.Ejecutar strSql
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    strSql = " INSERT INTO proceso_importacion" & _
                             " (pro_imp_codigo,pro_imp_nombre,pro_imp_descripcion,pro_imp_orden,pro_imp_fechamod,pro_imp_usumod) " & _
                             " VALUES ('" & UCase(VSFG.TextMatrix(i, 1)) & "','" & UCase(VSFG.TextMatrix(i, 2)) & "','" & UCase(VSFG.TextMatrix(i, 3)) & "'," & VSFG.TextMatrix(i, 4) & "," & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            strSql = " SELECT count(pro_imp_codigo) as existe " & _
