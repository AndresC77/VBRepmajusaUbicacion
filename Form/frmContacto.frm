...
Begin VB.Form frmContacto 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CheckBox chkFiltroNombre 
         BackColor       =   &H00DDDDDD&
         Caption         =   "Filtrar Nombre del Contacto"
         BeginProperty Font 
            Name            =   "Arial"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmContacto.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Private Sub Carga()
    ...
    strSql = " SELECT con_codigo,con_apellido,con_nombre,con_cargo,con_telf,con_fax,con_email," & _
             " con_fechamod, con_usumod, '0' as modi" & _
             " FROM contacto " & _
             " WHERE emp_codigo LIKE '" & strEmpresa & "' " & _
             " AND per_codigo='" & CodPer & "'"
  
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  CONCAT(con_apellido,' ',con_nombre) LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY CONCAT(con_apellido,' ',con_nombre) "
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            strSql = " UPDATE contacto " & _
                     " SET con_apellido='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                     " con_nombre='" & UCase(VSFG.TextMatrix(i, 3)) & "'," & _
                     " con_cargo='" & UCase(VSFG.TextMatrix(i, 4)) & "'," & _
                     " con_telf='" & VSFG.TextMatrix(i, 5) & "'," & _
                     " con_fax='" & VSFG.TextMatrix(i, 6) & "'," & _
                     " con_email='" & VSFG.TextMatrix(i, 7) & "'," & _
                     " con_fechamod=CURRENT_TIMESTAMP," & _
                     " con_usumod='" & strUsuario & "' " & _
                     " WHERE con_codigo='" & VSFG.TextMatrix(i, 1) & "' AND emp_codigo='" & strEmpresa & "' AND per_codigo='" & CodPer & "'"
             clsCon_Def.Ejecutar strSql, "M"
        'insert
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            'controla que este lleno los datos
            If VSFG.TextMatrix(i, 2) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta el apellido", vbInformation, "Ingreso"
                control = 1
            ...
            ElseIf VSFG.TextMatrix(i, 4) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta cargo", vbInformation, "Ingreso"
                control = 1
            Else
                strSql = " SELECT COALESCE(MAX(con_codigo),0)+1 as num " & _
                         " FROM contacto " & _
                ...
                'controla que no se repita el código
                     ...
                     strSql = " INSERT INTO contacto " & _
                              " (emp_codigo,per_codigo,con_codigo,con_apellido,con_nombre,con_cargo,con_telf," & _
                              " con_fax,con_email,con_fechamod,con_usumod) " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            strSql = " DELETE " & _
                     " FROM contacto " & _
                     " WHERE con_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
                     " AND emp_codigo='" & strEmpresa & "' AND per_codigo='" & CodPer & "'"
            clsCon_Def.Ejecutar (strSql), "M"
