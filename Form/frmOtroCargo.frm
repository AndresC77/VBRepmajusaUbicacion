...
Begin VB.Form frmOtroCargo 
   BackColor       =   &H00DDDDDD&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Recargos para Movimientos"
   ClientHeight    =   6960
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10155
   Icon            =   "frmOtroCargo.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmOtroCargo.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmOtroCargo"
...
Private Sub Carga()
    ...
    strSql = " SELECT oca_codigo,oca_nombre,oca_precio,oca_ctaconta, " & _
             " oca_fechamod, oca_usumod, '0' as modi" & _
             " FROM ocargos " & _
             " WHERE emp_codigo='" & strEmpresa & "' "
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  oca_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  oca_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY oca_nombre "
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            strSql = " UPDATE ocargos " & _
                 " SET oca_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                 " oca_precio=" & VSFG.TextMatrix(i, 3) & ", " & _
                 " oca_ctaconta='" & VSFG.TextMatrix(i, 4) & "'," & _
                 " oca_fechamod=CURRENT_TIMESTAMP," & _
                 " oca_usumod='" & strUsuario & "' " & _
                 " WHERE oca_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            Else
                ...
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    'Si no existe ese código de depósito inserta un nuevo registro en la tabla de depósito
                    strSql = " INSERT INTO ocargos(oca_codigo,emp_codigo,oca_nombre,oca_precio,oca_ctaconta,oca_fechamod,oca_usumod) " & _
                    " VALUES ('" & UCase(VSFG.TextMatrix(i, 1)) & "','" & strEmpresa & "','" & UCase(VSFG.TextMatrix(i, 2)) & "','" & VSFG.TextMatrix(i, 3) & "', " & _
                    " '" & VSFG.TextMatrix(i, 4) & "',CURRENT_TIMESTAMP, '" & strUsuario & "')"
                    clsCon_Def.Ejecutar strSql, "M"
                Else
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            strSql = " SELECT count(oca_codigo) as existe " & _
                     " FROM det_egreso_c " & _
                     " WHERE oca_codigo = '" & VSFG.TextMatrix(i, 1) & "' " & _
                     " AND emp_codigo='" & strEmpresa & "'"
            clsCon_Def.Ejecutar (strSql)
            ...
            Else
                 strSql = " SELECT count(oca_codigo) as existe2 " & _
