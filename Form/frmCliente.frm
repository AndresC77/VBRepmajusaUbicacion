...
Begin VB.Form frmCliente 
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmCliente.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Private Sub Carga()
    strSql = " SELECT per_codigo,per_cm,per_rcm,per_tipo,per_apellido,per_nombre,cat_p_codigo,can_codigo,per_ruc,ciu_codigo,zon_codigo," & _
             ...
             " per_sec_publico,per_siniva,per_inactivo,per_aplica_nc,per_perdesde,per_fechamod,per_usumod,per_fechaing,per_usuing, '0' as modi " & _
             " FROM persona" & _
    ...
    strSql = " SELECT cat_p_codigo, cat_p_nombre,cat_p_ing" & _
                 " FROM categoria_p " & _
                 " WHERE cat_p_tipo='C' " & _
                 " AND emp_codigo='" & strEmpresa & "' " & _
                 " ORDER BY cat_p_nombre"
     clsCon_Def.Ejecutar strSql
    VSFG.ColComboList(7) = VSFG.BuildComboList(clsCon_Def.adorec_Def, " *cat_p_nombre,cat_p_ing", "cat_p_codigo")
    'crea combo de categoria
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            ...
            If entra = 0 Then
                ...
                strSql = strSql & " ven_codigo='" & VSFG.TextMatrix(i, 24) & "'," & _
                     ...
                     " per_aplica_nc='" & Abs(FormatoD0(VSFG.TextMatrix(i, 37))) & "'," & _
...
'                control = 1
            ElseIf VSFG.TextMatrix(i, 7) = "" Then
                ...
                control = 1
            ElseIf Trim(VSFG.TextMatrix(i, 8)) = "" Then
                ...
                Else
                    ...
                    'controla que no se repita el código
                        strSql = " INSERT INTO persona(emp_codigo,per_codigo,per_cm,per_rcm,cat_p_tipo,per_tipo,per_apellido,per_nombre," & _
                                 ...
                                 " '" & Abs(FormatoD0(VSFG.TextMatrix(i, 31))) & "','" & Abs(FormatoD0(VSFG.TextMatrix(i, 34))) & "','" & Abs(FormatoD0(VSFG.TextMatrix(i, 35))) & "'," & _
                                 " CURRENT_TIMESTAMP, '" & strUsuario & "',CURRENT_TIMESTAMP, '" & strUsuario & "',CURRENT_DATE,'" & Abs(FormatoD0(VSFG.TextMatrix(i, 36))) & "','" & Abs(FormatoD0(VSFG.TextMatrix(i, 37))) & "')"
                        clsCon_Def.Ejecutar strSql, "M"
'                        If MsgBox("Desea registrar contactos al cliente" & vbNewLine & UCase(VSFG.TextMatrix(i, 5)) & " " & UCase(VSFG.TextMatrix(i, 6)) & "?", vbQuestion + vbYesNo, "Clientes") = vbYes Then
'                            frmContacto.CodPer = VSFG.TextMatrix(i, 1)
'                            frmContacto.Top = frmCliente.Top
'                            frmContacto.Show 1
'                        End If
                End If
             End If
...
Private Sub VSFG_BeforeEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    If Val(VSFG.TextMatrix(Row, VSFG.Cols - 1)) = 0 Or Val(VSFG.TextMatrix(Row, VSFG.Cols - 1)) = 1 Then
        If Col <> 30 Then
            Cancel = True ' VSFG_CellButtonClick Row, Col
        End If
        
    ElseIf Val(VSFG.TextMatrix(Row, VSFG.Cols - 1)) = 2 Or Val(VSFG.TextMatrix(Row, VSFG.Cols - 1)) = -2 Then
        VSFG.TextMatrix(Row, 20) = 0
        VSFG.TextMatrix(Row, 21) = 0
        VSFG.TextMatrix(Row, 22) = "EFE"
        VSFG.TextMatrix(Row, 8) = "VPC"
        VSFG.TextMatrix(Row, 37) = 0
        If Col = 8 Or Col = 20 Or Col = 21 Or Col = 22 Or Col >= 32 Then
            Cancel = True
        End If
    ElseIf Val(VSFG.TextMatrix(Row, VSFG.Cols - 1)) = 3 Or Val(VSFG.TextMatrix(Row, VSFG.Cols - 1)) = -3 Then
        If Col = 1 Or Col = 3 Or Col = 8 Or Col = 20 Or Col = 21 Or Col = 22 Or Col >= 32 Then
            Cancel = True
        End If
    End If
...
Private Sub VSFG_AfterEdit(ByVal Row As Long, ByVal Col As Long)
    If Val(VSFG.TextMatrix(Row, VSFG.Cols - 1)) = -2 Then
        ...
        If VSFG.TextMatrix(Row, 38) = "" Then
            VSFG.TextMatrix(Row, 38) = HoyDia
        End If
    ...
    If Col = 7 Then
        'MsgBox "AAA"
        strSql = "SELECT COALESCE(cat_p_ing,0) as s FROM categoria_p WHERE emp_codigo='" & strEmpresa & "' AND cat_p_codigo='" & VSFG.TextMatrix(Row, Col) & "'"
        clsCon_Def.Ejecutar strSql
        If FormatoD0(clsCon_Def.adorec_Def("s")) = 0 Then
            VSFG.TextMatrix(Row, Col) = ""
            MsgBox "Esa categoría no es permitida escojer", vbCritical, "Categoria"
        End If
    End If
    If Col = 9 Then
    ...
    If Col = 24 Then
        'MsgBox "AAA"
        strSql = "SELECT COALESCE(ven_ing,0) as s FROM vendedor WHERE emp_codigo='" & strEmpresa & "' AND ven_codigo='" & VSFG.TextMatrix(Row, Col) & "'"
        clsCon_Def.Ejecutar strSql
        If FormatoD0(clsCon_Def.adorec_Def("s")) = 0 Then
            VSFG.TextMatrix(Row, Col) = ""
            MsgBox "Ese vendedor no es permitido escojer", vbCritical, "Categoria"
        End If
    End If
    If Col = 18 Then
