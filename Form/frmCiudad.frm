...
Begin VB.Form frmCiudad 
   ...
   Begin VB.Frame Frame1 
      ...
      Top             =   120
      Width           =   7905
      Begin VB.CheckBox chkFiltroPais 
         BackColor       =   &H00DDDDDD&
         Caption         =   "Filtrar País"
         BeginProperty Font 
            Name            =   "Arial"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmCiudad.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
End Sub
Private Sub Carga()
    strSql = " SELECT ciu_codigo,ciu_nombre,pai_codigo,can_codigo,ciu_codigo_din, ciu_fechamod, ciu_usumod, '0' as modi " & _
             " FROM ciudad " & _
             " WHERE ciu_codigo LIKE '%'"
    If chkFiltroCodigo.Value = 1 Then
        strSql = strSql & "AND  ciu_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.Value = 1 Then
        strSql = strSql & " AND  ciu_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    If chkFiltroPais.Value = 1 Then
        strSql = strSql & " AND  pai_codigo = '" & dcmbPais.BoundText & "' "
    End If
    strSql = strSql & " ORDER BY ciu_nombre "
    ...
    strSql = " SELECT pai_codigo, pai_nombre" & _
                 ...
                 " ORDER BY pai_nombre"
     clsCon_Def.Ejecutar strSql
    VSFG.ColComboList(3) = VSFG.BuildComboList(clsCon_Def.adorec_Def, " *pai_nombre", "pai_codigo")
    'crea combo de pais
    strSql = " SELECT can_codigo, can_nombre" & _
                 " FROM canton " & _
                 " ORDER BY can_nombre"
     clsCon_Def.Ejecutar strSql
    VSFG.ColComboList(4) = VSFG.BuildComboList(clsCon_Def.adorec_Def, " *can_nombre", "can_codigo")
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            strSql = " UPDATE ciudad " & _
                 " SET ciu_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "',pai_codigo='" & VSFG.TextMatrix(i, 3) & "'," & _
                 " can_codigo='" & UCase(VSFG.TextMatrix(i, 4)) & "'," & _
                 " ciu_codigo_din='" & UCase(VSFG.TextMatrix(i, 5)) & "'," & _
                 " ciu_fechamod=CURRENT_TIMESTAMP," & _
                 " ciu_usumod='" & strUsuario & "' " & _
                 " WHERE ciu_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                 " "
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 3) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta país", vbInformation, "Ingreso"
                control = 1
            ElseIf VSFG.TextMatrix(i, 4) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta cantón", vbInformation, "Ingreso"
                control = 1
            Else
                strSql = " SELECT ciu_codigo" & _
                    " FROM ciudad " & _
                    " WHERE ciu_codigo='" & VSFG.TextMatrix(i, 1) & "'"
                clsCon_Def.Ejecutar strSql
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    strSql = " INSERT INTO ciudad(ciu_codigo,ciu_nombre,pai_codigo,can_codigo,ciu_codigo_din,ciu_fechamod,ciu_usumod) " & _
                            " VALUES ('" & UCase(VSFG.TextMatrix(i, 1)) & "','" & UCase(VSFG.TextMatrix(i, 2)) & "','" & VSFG.TextMatrix(i, 3) & "','" & VSFG.TextMatrix(i, 4) & "','" & VSFG.TextMatrix(i, 5) & "', " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            ...
            Else
                strSql = " DELETE " & _
                    " FROM ciudad " & _
                    " WHERE ciu_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
                    " AND pai_codigo='" & VSFG.TextMatrix(i, 3) & "'"
                clsCon_Def.Ejecutar (strSql), "M"
...
Private Sub chkFiltroCodigo_Click()
    ...
    End If
End Sub
Private Sub chkFiltroPais_Click()
 If chkFiltroPais = 0 Then
    dcmbPais.Enabled = True
 Else
    strSql = " SELECT pai_nombre,pai_codigo " & _
             " FROM pais " & _
             " ORDER BY pai_nombre "
    clsCon_Def.Ejecutar strSql
    Set dcmbPais.RowSource = clsCon_Def.adorec_Def.DataSource
