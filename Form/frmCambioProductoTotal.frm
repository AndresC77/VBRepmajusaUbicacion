...
Begin VB.Form frmCambioProductoTotal 
   BackColor       =   &H00DDDDDD&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Cambio de Productos Total"
   ClientHeight    =   5610
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   12600
   Icon            =   "frmCambioProductoTotal.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   ScaleHeight     =   5610
   ScaleWidth      =   12600
   Begin VB.CommandButton cmdImprimirCambio 
      Caption         =   "&Imprimir Cambio"
      Height          =   375
      Left            =   10680
      TabIndex        =   19
      Top             =   1080
   ...
   Begin VB.Frame Frame3 
      ...
      Begin VSFlex8LCtl.VSFlexGrid vsfgDetalleIng 
         ...
         FormatString    =   $"frmCambioProductoTotal.frx":030A
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   Begin VB.Image imgBtnDn 
      Height          =   210
      Left            =   600
      Picture         =   "frmCambioProductoTotal.frx":04CB
      Top             =   5040
      Visible         =   0   'False
      Width           =   225
   End
   Begin VB.Image imgBtnUp 
      Height          =   210
      Left            =   360
      Picture         =   "frmCambioProductoTotal.frx":05F7
      Top             =   5040
      Visible         =   0   'False
      Width           =   225
   End
End
Attribute VB_Name = "frmCambioProductoTotal"
...
Private Sub cmbNegocio_Change()
    ...
    strSql = " SELECT CONCAT(per_apellido,' ',per_nombre,' (',per_ruc,')') as nombC, COALESCE(CONCAT(ven_apellido,' ',ven_nombre),'') as nombV, " & _
             ...
             " COALESCE(CONCAT(per_telf,'/',per_fax),'') as per_tf,per_observacion,cat_p_dcto,per_dcto,per_credito,(persona.per_bloqueado+persona.per_bloqueado_g) as per_bloqueado,per_codigo_ref,per_codigo_ref2 " & _
...
Private Sub cmdAceptar_Click()
    ...
    GenerarCambio clsCambio.strDoc
    GenerarNC clsCambio.strDoc
...
Private Sub GenerarNC(DocCambio As String)
    Dim i As Long
    Dim booGuardar As Boolean
    booGuardar = False
    
    For i = 1 To vsfgDetalleIng.Rows - 1
        If (Trim(vsfgDetalleIng.TextMatrix(i, 6)) = "" And FormatoD0(vsfgDetalleIng.TextMatrix(i, 4)) > 0) Or (Trim(vsfgDetalleIng.TextMatrix(i, 6)) <> "" And FormatoD0(vsfgDetalleIng.TextMatrix(i, 4)) > 0 And FormatoD0(vsfgDetalleIng.TextMatrix(i, 10)) < FormatoD0(vsfgDetalleIng.TextMatrix(i, 4))) Then
                booGuardar = True
                Exit For
        End If
    Next i
    ...
    If booGuardar = True Then
        ...
        frmRealizaDevolucionProducto.cmbNegocio.BoundText = cmbNegocio.BoundText
        frmRealizaDevolucionProducto.cmbCliente.BoundText = cmbCliente.BoundText
        frmRealizaDevolucionProducto.cmbFormulario.BoundText = DocCambio
        frmRealizaDevolucionProducto.txtNumAux.Text = cmbFactura.Text
        frmRealizaDevolucionProducto.cmdBuscar_Click
        frmRealizaDevolucionProducto.TxtObserv.Text = TxtObserv.Text
        frmRealizaDevolucionProducto.CmbFpago.BoundText = "CONT"
        For i = 1 To frmRealizaDevolucionProducto.VSFG.Rows - 1
            frmRealizaDevolucionProducto.VSFG.TextMatrix(i, 0) = 1
        Next i
    End If
End Sub

Private Sub GenerarCambio(DocCambio As String)
    Dim clsIngreso As New clsInventario
    Dim clsEgreso As New clsInventario
    Dim clsCambio As New clsCambio
    Dim rpTra As New frmReporte
    Dim booGuardar As Boolean
    Dim strBodegaA As String
    Dim strBodegaDesde As String
    ...
    For i = 1 To vsfgDetalleIng.Rows - 1
        If Trim(vsfgDetalleIng.TextMatrix(i, 6)) <> "" And FormatoD0(vsfgDetalleIng.TextMatrix(i, 4)) > 0 Then
            If FormatoD0(vsfgDetalleIng.TextMatrix(i, 4)) < FormatoD0(vsfgDetalleIng.TextMatrix(i, 10)) Then
                booGuardar = True
                Exit For
            End If
        End If
    ...
    If booGuardar = True Then
        ...
        If booGuardar = True Then
            ...
            With vsfgDetalleIng
                For i = 1 To .Rows - 1
                    If Trim(.TextMatrix(i, 6)) <> "" And FormatoD0(.TextMatrix(i, 4)) > 0 Then
                        strBodegaDesde = "PRI"
                        If .TextMatrix(i, 2) = .TextMatrix(i, 6) Then
                            MsgBox "Se va hacer el ingreso del producto " & .TextMatrix(i, 2) & " a la bodega de IMPERFECTOS", vbInformation, "Bodegas"
                            strBodegaA = "IMP" 'IMPERFECTOS
                        Else
                            strBodegaA = strBodegaDesde
                        End If
                        If FormatoD0(.TextMatrix(i, 4)) < FormatoD0(.TextMatrix(i, 10)) Then
                            clsIngreso.NuevoDetIng .TextMatrix(i, 2), strBodegaA, .TextMatrix(i, 4), FormatoD8(.TextMatrix(i, 11)), FormatoD4(.TextMatrix(i, 11)), 0, 1
                            strContenedorRecurrente = ""
                            clsEgreso.NuevoDetEgr .TextMatrix(i, 6), strBodegaDesde, .TextMatrix(i, 4), FormatoD4(.TextMatrix(i, 11)), FormatoD4(.TextMatrix(i, 11)), 0, 1
                            clsCambio.AsignarIngreso DocCambio, .TextMatrix(i, 1), .TextMatrix(i, 2), .TextMatrix(i, 6), clsIngreso.strTipo, clsIngreso.strDoc
                            .TextMatrix(i, 12) = "C"
                        End If
                    End If
                Next i
            ...
            End With
        End If
        MsgBox "El cambio ha sido ingresado" & vbNewLine & clsCambio.strDoc, vbInformation, "Cambio de Productos"
        
        rpTra.strNumero = clsIngreso.strDoc
        rpTra.strTipo = clsEgreso.strDoc
...
Private Sub cmdImprimirCambio_Click()
    Dim rpMov1 As New frmReporte
    Dim strCambio As String
    strCambio = InputBox("Ingrese el numero de Cambio", "Cambios")
    rpMov1.strNumero = strCambio
    rpMov1.strTipo = strCambio
    rpMov1.strReporte = "rptCambioProducto"
    rpMov1.Show
End Sub
...
Private Sub ConsultaProducto(PrdCodigo As String)
        ...
        If Left(cmbFactura.Text, 1) = "R" Then
            strSql = " SELECT producto.prd_codigo, prd_nombre,lis_pre_p_precio as prd_precio,det_egr_cantidad,producto.prd_sku,producto.prd_costo " & _
                     ...
                     " ORDER BY prd_nombre "
        Else
            strSql = " SELECT producto.prd_codigo, prd_nombre,det_egr_precio as prd_precio,det_egr_cantidad,producto.prd_sku,producto.prd_costo " & _
                     ...
                     " UNION " & _
                     " SELECT producto.prd_codigo, prd_nombre,lis_pre_p_precio as prd_precio,det_cam_cantidad,producto.prd_sku,producto.prd_costo " & _
...
Private Sub vsfgDetalleIng_BeforeRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long, Cancel As Boolean)
    If OldCol = 4 Then
        If FormatoD2(vsfgDetalleIng.TextMatrix(OldRow, 4)) > FormatoD2(vsfgDetalleIng.TextMatrix(OldRow, 9)) Then
...
Private Sub vsfgDetalleIng_CellChanged(ByVal Row As Long, ByVal Col As Long)
    If Col = 2 Then
        ...
        If clsCon_Prd.adorec_Def.RecordCount > 0 Then
            ...
            vsfgDetalleIng.TextMatrix(Row, 11) = clsCon_Prd.adorec_Def("prd_costo")
        Else
            ...
            vsfgDetalleIng.TextMatrix(Row, 11) = 0
        End If
    ElseIf Col = 6 Then
        ...
        End If
        If Trim(vsfgDetalleIng.TextMatrix(Row, 6)) <> "" Then
            vsfgDetalleIng.TextMatrix(Row, 10) = DisponibleParaLaVentaBodega(Trim(vsfgDetalleIng.TextMatrix(Row, 6)), "PRI")
        Else
            vsfgDetalleIng.TextMatrix(Row, 10) = 0
        End If
    ElseIf Col = 7 Then
