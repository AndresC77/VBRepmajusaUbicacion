...
Begin VB.Form frmV_VerPedBod 
   ...
   ScaleHeight     =   7950
   ScaleWidth      =   10785
   Begin VB.CommandButton cmdBajaGeneral 
      Caption         =   "Dar de Baja"
      Height          =   375
      Left            =   1680
      TabIndex        =   29
   ...
   Begin VB.Frame frmDet 
      ...
      Begin VSFlex8Ctl.VSFlexGrid VSFG 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   0   'False
         FormatString    =   $"frmVerPedBod.frx":030A
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   Begin VB.Frame frmPed 
      ...
      Begin VSFlex8Ctl.VSFlexGrid VSFGPeds 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   0   'False
         FormatString    =   $"frmVerPedBod.frx":03FA
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmV_VerPedBod"
    ...
    If cmbNegocio.BoundText <> "" Then
        ...
        If Me.optListaPedido.Value = True Then
            If clsSql.adorec_Def.RecordCount > 0 Then
                ...
                strSql = " SELECT RIGHT(pedido.ped_codigo,7)+0 as c,ped_fecha,ped_observacion," & strCli & " as nombC,pedido.ped_estado," & _
                         " cot_codigo,pedido.ped_estado,pedido.ven_codigo,ped_codigo," & _
                         " IIF(persona.per_bloqueado+persona.per_bloqueado_g=0,0,1) as per_bloqueado " & _
                         " FROM pedido INNER JOIN persona ON pedido.emp_codigo = persona.emp_codigo AND pedido.per_codigo = persona.per_codigo " & _
                         " Where pedido.emp_codigo='" & strEmpresa & "' AND cat_p_tipo='C' AND ped_estado in (0,1) AND " & _
                         " (ped_fecha=IIF(pedido.ped_estado=2,'" & Format(HoyDia, "yyyy-MM-dd") & "',ped_fecha)) " & _
                         " AND pedido.ped_codigo LIKE CONCAT('" & strSucursal & clsSql.adorec_Def(0) & "'+0,'%') " & _
                         " ORDER BY pedido.ped_estado,ped_codigo "
...
Private Sub cmdBajaGeneral_Click()
...
Private Sub txtLector_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyReturn Then
        ...
        txtLector.Text = ""
        CalculaCant
    ElseIf Shift = 5 And KeyCode = vbKeyF6 And VSFGPeds.TextMatrix(VSFGPeds.Row, 8) <> "" Then
        If MsgBox("Desea cambiar todo el pedido de bodega?", vbQuestion + vbYesNo, "Pedidos") = vbYes Then
            While strBodega = ""
                strBodega = InputBox("Ingrese el codigo de bodega", "Cambio de bodega", "")
                strSql = " SELECT dep_nombre FROM deposito WHERE emp_codigo='" & strEmpresa & "' AND dep_codigo='" & strBodega & "'"
                clsSql.Ejecutar (strSql)
                If clsSql.adorec_Def.RecordCount = 0 Then
                    
                    MsgBox "La bodega " & strBodega & " no existe", vbCritical, "Pedidos"
                    strBodega = ""
                End If
            Wend
            strSql = " UPDATE det_pedido " & _
                     " SET dep_codigo='" & strBodega & "' " & _
                     " WHERE emp_codigo='" & strEmpresa & "' " & _
                     " AND ped_codigo='" & VSFGPeds.TextMatrix(VSFGPeds.Row, 8) & "'"
            clsSql.Ejecutar (strSql), "M"
        End If
        If MsgBox("Desea confirmar el pedido a CANTIDAD?", vbQuestion + vbYesNo, "Pedidos") = vbYes Then
            
            strSql = " UPDATE det_pedido " & _
                     " SET " & _
                     " det_ped_cant_entregada=det_ped_cant_pedida, " & _
            ...
            clsSql.Ejecutar (strSql), "M"
        End If
        txtPedido.Text = VSFGPeds.TextMatrix(VSFGPeds.Row, 8)
        txtPedido_KeyDown vbKeyReturn, 0
    End If
End Sub
...
'****** PEDIDO
    'Da de baja al pedido
    strSql = " UPDATE pedido SET ped_estado=5, " & _
             " ped_fechamod=CURRENT_TIMESTAMP, " & _
             " ped_usumod='" & strUsuario & "' " & _
             " WHERE emp_codigo='" & strEmpresa & "' AND ped_codigo=" & LblPedido
    clsSql.Ejecutar (strSql), "M"
...
Private Sub CmdConfirmar_Click()
    ...
    Else
        ...
        strSql = " UPDATE pedido SET ped_estado=1, " & _
                 " ped_usumod='" & strUsuario & "', " & _
                 " ped_fechamod=CURRENT_TIMESTAMP,tipo_fac_codigo= " & tFac & _
                 " WHERE emp_codigo='" & strEmpresa & "' AND ped_codigo=" & LblPedido & " "
        clsSql.Ejecutar (strSql), "M"
        'Llama al botón limpiar grid
        'Actualiza el grid de pedidos
        'clsPedidosV.Actualizar
        'Set VSFGPeds.DataSource = clsPedidosV.adorec_Def.DataSource
        ...
        frmV_VerPedConfirm.txtPedido = FormatoD0(LblPedido)
        frmV_VerPedConfirm.txtPedido_KeyDown vbKeyReturn, 0
        frmV_VerPedConfirm.SetFocus
        If frmV_VerPedConfirm.CmdConfirmar.Visible = True Then
            frmV_VerPedConfirm.CmdConfirmar.SetFocus
        ElseIf frmV_VerPedConfirm.cmdNotaEntrega.Visible = True Then
            frmV_VerPedConfirm.cmdNotaEntrega.SetFocus
        End If
        CmdLimpiar = True
    End If
    'TmrAct.Enabled = True
  ...
  With VSFG
    For i = 1 To .Rows - 1
      If codigo = .TextMatrix(i, 1) Then
        .ShowCell i, 1
        .Select i, 1
        If Val(Format(.TextMatrix(i, 3), "###0.00")) < Val(Format(.TextMatrix(i, 4), "###0.00")) + 1 Then
          MsgBox "El número de productos con código " & codigo & " Excede la cantidad requerida.", vbInformation, "Cantidad excedida"
        ElseIf Val(Format(.TextMatrix(i, 5), "###0.00")) < Val(Format(.TextMatrix(i, 4), "###0.00")) + 1 Then
...
Private Sub Form_Load()
    ...
    strSql = " SELECT RIGHT(ped_codigo,7)+0 as c,ped_fecha,ped_observacion,CONCAT(per_apellido,' ',per_nombre) as nombC,ped_estado,cot_codigo,ped_estado,pedido.ven_codigo,ped_codigo,IIF(persona.per_bloqueado+persona.per_bloqueado_g=0,0,1) as per_bloqueado " & _
             ...
             " (ped_fecha=IIF(ped_estado=2,'" & Format(HoyDia, "yyyy-MM-dd") & "',ped_fecha)) " & _
        ...
        If cmbNegocio.BoundText <> "" Then
            ...
            If Me.optNoPedido.Value = True Then
                If clsSql.adorec_Def.RecordCount > 0 Then
                    banTm = 0
                    'Consulta todos los pedidos que pasan a bodega para ser revisados
                    strSql = " SELECT RIGHT(pedido.ped_codigo,7)+0 as c,ped_fecha,ped_observacion," & strCli & " as nombC,pedido.ped_estado,cot_codigo,pedido.ped_estado,pedido.ven_codigo,ped_codigo,IIF(persona.per_bloqueado+persona.per_bloqueado_g=0,0,1) as per_bloqueado " & _
...
''''ÁQUI**********************************
        strSql = " SELECT existencia.dep_codigo, existencia.prd_codigo, existencia.emp_codigo, " & _
                 " Sum(existencia.exi_cantidad) AS Exis " & _
                 " FROM existencia " & _
        ...
        strSql = " SELECT det_pedido.dep_codigo, det_pedido.prd_codigo, det_pedido.emp_codigo, " & _
                 " Sum(det_pedido.det_ped_cant_entregada) AS Exis " & _
...
Private Sub VSFGPeds_DblClick()
    ...
    If VSFGPeds.Row > 0 Then
        ...
        strSql = " SELECT dep_codigo, det_pedido.prd_codigo, prd_nombre, det_ped_cant_pedida, IIF(det_pedido.prd_codigo='PR-CARGOO100330TU',det_ped_cant_pedida,IIF(pedido.ped_codigo like '4002%' OR pedido.ped_codigo like '4001%',0,det_ped_cant_confirmada)) as det_ped_cant_confirmada, '' as HH,'' as Descripcion " & _
                 ...
                 " Where pedido.emp_codigo='" & strEmpresa & "' AND " & _
                 " det_pedido.ped_codigo= " & VSFGPeds.TextMatrix(VSFGPeds.Row, 8) & _
        ...
        'Cliente Bloqueado
        If Abs(VSFGPeds.TextMatrix(VSFGPeds.Row, 9)) = 1 Then
            CmdConfirmar.Enabled = False
            CmdDeBaja.Enabled = False
            MsgBox "Cliente Bloqueado" & vbNewLine & "El pedido se anulara por bloqueo", vbInformation, "Bloqueo"
            ...
            '****** PEDIDO
                ...
                clsSql.Ejecutar (strSql), "M"
                'Limpia los grids que mostraban datos del pedido
                MsgBox "Pedido No. " & LblPedido & " dado de Baja por Bloqueo.", vbInformation, "De Baja"
                CmdLimpiar = True
                'Actualiza el grid que muestra los pedidos actuales
                clsPedidosV.Actualizar
                Set VSFGPeds.DataSource = clsPedidosV.adorec_Def.DataSource
        ...
        Else
            ...
            If VSFGPeds.TextMatrix(VSFGPeds.Row, 4) <> 1 And VSFGPeds.TextMatrix(VSFGPeds.Row, 4) <> 0 Then
            ...
            ElseIf VSFGPeds.TextMatrix(VSFGPeds.Row, 4) = 0 Then
