...
Begin VB.Form frmProductoClc 
   BackColor       =   &H00DDDDDD&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Productos - Colección"
   ClientHeight    =   7515
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   12285
   Icon            =   "frmProductoClc.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmProductoClc.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   Begin VB.Image imgBtnUp 
      Height          =   240
      Left            =   5760
      Picture         =   "frmProductoClc.frx":0543
   ...
   End
End
Attribute VB_Name = "frmProductoClc"
...
Private Sub Carga()
    strSQL = " SELECT prd_codigo,prd_nombre,mar_codigo,lin_codigo,gru_codigo,uni_codigo,tal_codigo,col_codigo," & _
             " clc_codigo,prd_baja,'' as precio,'' as promo,prd_costo,prd_cambia_precio,prd_iva,prd_fechamod,prd_usumod, '0' as modi " & _
             " FROM producto" & _
            ...
            Else
                ...
                'controla que no se repita el código
                    strSQL = " INSERT INTO producto(emp_codigo,prd_codigo,prd_nombre,mar_codigo,lin_codigo,gru_codigo,uni_codigo,tal_codigo,col_codigo,clc_codigo,prd_baja," & _
                             " prd_costo,prd_cambia_precio,prd_iva,prd_fechamod,prd_usumod) " & _
                             " VALUES ('" & strEmpresa & "','" & UCase(VSFG.TextMatrix(i, 1)) & "','" & VSFG.TextMatrix(i, 2) & "','" & UCase(VSFG.TextMatrix(i, 3)) & "','" & UCase(VSFG.TextMatrix(i, 4)) & "', " & _
                             " '" & VSFG.TextMatrix(i, 5) & "','" & VSFG.TextMatrix(i, 6) & "','" & VSFG.TextMatrix(i, 7) & "','" & VSFG.TextMatrix(i, 8) & "','" & VSFG.TextMatrix(i, 9) & "','" & Abs(FormatoD0(VSFG.TextMatrix(i, 10))) & "','" & FormatoD4(VSFG.TextMatrix(i, 13)) & "','" & Abs(FormatoD0(VSFG.TextMatrix(i, 14))) & "','" & Abs(FormatoD0(VSFG.TextMatrix(i, 15))) & "'," & _
