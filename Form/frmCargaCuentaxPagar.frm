...
Begin VB.Form frmCargaCuentaxPagar 
   BackColor       =   &H00DDDDDD&
   ...
   ClientTop       =   330
   ClientWidth     =   13005
   Icon            =   "frmCargaCuentaxPagar.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Frame Frame3 
      ...
      Begin VB.Frame Frame2 
         ...
         Begin VSFlex8Ctl.VSFlexGrid VSFG 
            ...
            FormatString    =   $"frmCargaCuentaxPagar.frx":030A
            ScrollTrack     =   0   'False
            ScrollBars      =   3
            ScrollTips      =   0   'False
            MergeCells      =   0
      ...
      Begin MSDataListLib.DataCombo dcmbIVA 
         ...
         Text            =   ""
      End
      Begin MSDataListLib.DataCombo dcmbCentroCosto 
         Height          =   315
         Left            =   8280
         TabIndex        =   5
         Top             =   720
   ...
   End
End
Attribute VB_Name = "frmCargaCuentaxPagar"
...
Private Sub cmdAplicar_Click()
    ...
    Dim RepRet As New frmReporte
    Dim RepAsi As New frmReporte
 
 
    TotalCuenta = 0
    ban = True
    CuentaContaGasto = dtcm_CuentaContable.BoundText
    CentroCosto = dcmbCentroCosto.BoundText
    If CentroCosto = "" And (Left(CuentaContaGasto, 1) = "4" Or Left(CuentaContaGasto, 1) = "5" Or Left(CuentaContaGasto, 1) = "6") Then
        ...
        For i = 1 To VSFG.Rows - 1
            If (VSFG.TextMatrix(i, 0) <> "") Then
                If (VSFG.TextMatrix(i, 17) = 1) Then
                    'Variables del grid
                    Total_debe = FormatoD2(VSFG.TextMatrix(i, 7))
                    
                    'Busca el código máximo de la tabla asiento
                    clsAsiento.Inicializar AdoConn, AdoConnMaster
                    clsAsiento.NuevoAsiento "D", Format(VSFG.TextMatrix(i, 8), "yyyy-mm-dd"), 0, 0, FormatoD2(Total_debe), _
                    ...
                    clsCxX.NuevaCta "P", dcmbTipoDoc.BoundText, dcmbSustento.BoundText, Format(VSFG.TextMatrix(i, 8), "yyyy-mm-dd"), Format(VSFG.TextMatrix(i, 8), "yyyy-mm-dd"), VSFG.TextMatrix(i, 16), VSFG.TextMatrix(i, 14), VSFG.TextMatrix(i, 1), VSFG.TextMatrix(i, 2), VSFG.TextMatrix(i, 3), Format(VSFG.TextMatrix(i, 4), "mm/yyyy"), 0, FormatoD2(VSFG.TextMatrix(i, 5)), 0, IIf(FormatoD2(VSFG.TextMatrix(i, 6)) > 0, FormatoD2(VSFG.TextMatrix(i, 5)), 0), dcmbIVA.BoundText, FormatoD2(VSFG.TextMatrix(i, 6)), IIf(FormatoD2(VSFG.TextMatrix(i, 6)) > 0, 0, FormatoD2(VSFG.TextMatrix(i, 5))), 0, 0, 0, FormatoD2(VSFG.TextMatrix(i, 7)), clsAsiento.NumAsiento
                    CuentaXPagar = clsCxX.strNoCta
                    
                    'Cuentas y valores a insertar
                    
                    d_imprenta = FormatoD2(FormatoD2(VSFG.TextMatrix(i, 5)) * FormatoD2(VSFG.TextMatrix(i, 21)) / 100#)
                    d_iva = FormatoD2(FormatoD2(VSFG.TextMatrix(i, 6)) * FormatoD2(VSFG.TextMatrix(i, 23)) / 100#)
                    d_clienteprov = FormatoD2((FormatoD2(VSFG.TextMatrix(i, 7)) - d_iva - d_imprenta))
                    d_comisiones = FormatoD2((FormatoD2(VSFG.TextMatrix(i, 7)) - FormatoD2(VSFG.TextMatrix(i, 6))))
                    
                    'Inserta el detalle de los asientos
                    clsAsiento.NuevoDetAsiento VSFG.TextMatrix(i, 19), "", FormatoD2(0), FormatoD2(d_clienteprov)
                    If (Trim(VSFG.TextMatrix(i, 6)) <> "") Then
                        clsAsiento.NuevoDetAsiento "1.01.08.01.001", "", FormatoD2(VSFG.TextMatrix(i, 6)), FormatoD2(0)
                    End If
                    clsAsiento.NuevoDetAsiento CuentaContaGasto, CentroCosto, FormatoD2(d_comisiones), FormatoD2(0)
                    ...
                    strSql = " SELECT TOP 1 com_ret_serie,com_ret_numero " & _
                             " FROM comprobante_retencion  " & _
                             " WHERE " & IIf(GeneraDocElec = 1, "com_ret_serie='001" & PtoEmiDocEle & "' AND ", "") & " emp_codigo = '" & strEmpresa & "' AND cue_p_c_tipo='P' ORDER BY com_ret_numero DESC"
                    clsSql.Ejecutar strSql, "M"
                    If Not clsSql.adorec_Def.EOF Then
                        VSFG.TextMatrix(i, 9) = IIf(GeneraDocElec = 1, "001" & PtoEmiDocEle & "", "")
                        VSFG.TextMatrix(i, 11) = "0"
                        VSFG.TextMatrix(i, 10) = FormatoD2(clsSql.adorec_Def("com_ret_numero")) + 1
                    Else
                        VSFG.TextMatrix(i, 9) = IIf(GeneraDocElec = 1, "001" & PtoEmiDocEle & "", "")
                        VSFG.TextMatrix(i, 11) = "0"
                        VSFG.TextMatrix(i, 10) = 1
                    End If
                    ...
                    If (Trim(VSFG.TextMatrix(i, 13)) <> "") Then
                        ...
                        " VALUES ('" & strEmpresa & "','" & CuentaXPagar & "','" & Tipo & "', '" & Trim(VSFG.TextMatrix(i, 13)) & "', " & _
                    ...
                    If (Trim(VSFG.TextMatrix(i, 12)) <> "") Then
                        strSql = " INSERT INTO det_comp_ret (emp_codigo,cue_p_c_codigo,cue_p_c_tipo,ret_codigo,det_com_ret_valor,det_com_ret_porcentaje,det_com_ret_fechamod,det_com_ret_usumod) " & _
                        " VALUES ('" & strEmpresa & "','" & CuentaXPagar & "','" & Tipo & "', '" & Trim(VSFG.TextMatrix(i, 12)) & "', " & _
                        FormatoD2(VSFG.TextMatrix(i, 5)) & "," & FormatoD2(VSFG.TextMatrix(i, 21)) & ", CURRENT_TIMESTAMP, '" & strUsuario & "')"
                        clsSql.Ejecutar strSql, "M"
                    End If
                    DocElectronico "07", (CuentaXPagar)
                    
                    RepRet.strNumero = CuentaXPagar
                    RepRet.strTipo = Tipo
                    RepRet.strAsiento = clsAsiento.NumAsiento
                    RepRet.strReporte = "rptRetencionDiario"
                    RepRet.Show
                    RepRet.Form_Activate
                    RepRet.VSPrint.PrintDoc
         ...
         If (VSFG.TextMatrix(i, 0) <> "") Then
           i_cont = i_cont + 1
            strSql = " SELECT per_codigo, per_ruc, CONCAT(per_apellido,per_nombre) as Proveedor" & _
                     ...
                     " AND cat_p_tipo='P' AND per_ruc='" & VSFG.TextMatrix(i, 0) & "' "
            ...
            strSql = " SELECT c.cat_p_ctaconta, p.per_ruc" & _
                     ...
                     " AND p.cat_p_tipo='P' AND p.per_ruc='" & VSFG.TextMatrix(i, 0) & "' "
            clsCon_Def.Ejecutar strSql
            ...
            strSql = " SELECT ret_codigo, ret_ctaconta, ret_porcentaje" & _
                     " FROM retencion " & _
                     " WHERE emp_codigo='" & strEmpresa & "'" & _
                     " AND ret_codigo='" & Trim(VSFG.TextMatrix(i, 12)) & "' "
            clsCon_Def.Ejecutar strSql
            ...
            strSql = " SELECT ret_codigo, ret_ctaconta, ret_porcentaje" & _
                     " FROM retencion " & _
                     " WHERE emp_codigo='" & strEmpresa & "'" & _
                     " AND ret_codigo='" & Trim(VSFG.TextMatrix(i, 13)) & "' "
            clsCon_Def.Ejecutar strSql
...
Private Sub Form_Load()
    ...
    strSql = " SELECT cen_cos_codigo,cen_cos_nombre " & _
     ...
     Set dcmbCentroCosto.RowSource = clsAsi.adorec_Def.DataSource
     dcmbCentroCosto.ListField = "cen_cos_nombre"
     dcmbCentroCosto.BoundColumn = "cen_cos_codigo"
    ...
    strSql = " SELECT cod_iva_codigo, cod_iva_porcentaje" & _
     ...
     clsAsi.Ejecutar (strSql)
     au = clsAsi.adorec_Def("cod_iva_codigo")
     
    strSql = " SELECT cod_iva_codigo, cod_iva_porcentaje" & _
                 " FROM codigo_iva " & _
                 " ORDER BY cod_iva_porcentaje"
     clsAsi.Ejecutar (strSql)
     Set dcmbIVA.RowSource = clsAsi.adorec_Def.DataSource
