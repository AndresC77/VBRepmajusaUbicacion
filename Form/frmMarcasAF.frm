...
Begin VB.Form frmMarcasAF 
   BackColor       =   &H00DDDDDD&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Marcas de Activos Fijos"
   ClientHeight    =   6960
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8280
   Icon            =   "frmMarcasAF.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmMarcasAF.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Private Sub IniDato()
    Tipo = "Marcas de Activos Fijos"
    Tipo2 = "Marcas de Activos Fijos"
    Me.Caption = Tipo
End Sub
...
End Sub
Private Sub Carga()
    strSql = " SELECT mar_act_fij_codigo,mar_act_fij_nombre,mar_act_fij_fechamod,mar_act_fij_usumod, '0' as modi " & _
             " FROM marca_activo_fijo " & _
             " WHERE 1=1 "
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  mar_act_fij_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  mar_act_fij_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY mar_act_fij_codigo,mar_act_fij_nombre "
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
           ...
           strSql = " UPDATE marca_activo_fijo " & _
                 " SET mar_act_fij_nombre='" & UCase(Trim(VSFG.TextMatrix(i, 2))) & "'," & _
                 " mar_act_fij_fechamod=CURRENT_TIMESTAMP," & _
                 " mar_act_fij_usumod='" & strUsuario & "' " & _
                 " WHERE mar_act_fij_codigo='" & VSFG.TextMatrix(i, 1) & "' "
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf Trim(VSFG.TextMatrix(i, 2)) = "" Then
                ...
                control = 1
            Else
                strSql = " SELECT mar_act_fij_codigo" & _
                    " FROM marca_activo_fijo " & _
                    " WHERE mar_act_fij_codigo='" & Trim(VSFG.TextMatrix(i, 1)) & "'"
                ...
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    ...
                    strSql = " INSERT INTO marca_activo_fijo(mar_act_fij_codigo,mar_act_fij_nombre,mar_act_fij_fechamod,mar_act_fij_usumod) " & _
                            " VALUES ('" & UCase(Trim(VSFG.TextMatrix(i, 1))) & "','" & UCase(Trim(VSFG.TextMatrix(i, 2))) & "', " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            strSql = " SELECT count(*) As existe " & _
                " FROM activo_fijo " & _
                " WHERE mar_act_fij_codigo='" & Trim(VSFG.TextMatrix(i, 1)) & "'" & _
