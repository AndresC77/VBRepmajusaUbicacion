...
Begin VB.Form frmV_RangoComi2 
   BackColor       =   &H00DDDDDD&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Comision por Utilidad Bruta sobre Venta"
   ClientHeight    =   5055
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   4350
   Icon            =   "frmV_RangoComi2.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VSFlex8Ctl.VSFlexGrid VSFG 
         ...
         FormatString    =   $"frmV_RangoComi2.frx":030A
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
      ...
      Begin VB.Image imgBtnDn 
         Height          =   210
         Left            =   360
         Picture         =   "frmV_RangoComi2.frx":03A9
         Top             =   2520
         Visible         =   0   'False
         Width           =   225
      End
      Begin VB.Image imgBtnUp 
         Height          =   210
         Left            =   120
         Picture         =   "frmV_RangoComi2.frx":04D5
   ...
   End
End
Attribute VB_Name = "frmV_RangoComi2"
...
Private Sub cmdAceptar_Click()
    ...
    Else
        ...
        clsSql.Ejecutar strSql, "M"
    End If
    strSql = " INSERT INTO rango_comi2 (emp_codigo,ran_com_codigo,ran_com_fecha,ran_com_fechafin,ran_com_activo,ran_com_fechamod,ran_com_usumod)" & _
             " VALUES('" & strEmpresa & "','" & lonCod & "','" & DTPicker1.value & "','" & _
             DTPicker2.value & "','" & intAct & "',CURRENT_TIMESTAMP,substring_index(USER(),'@',1))"
    clsSql.Ejecutar strSql, "M"
    For i = 1 To VSFG.Rows - 1
        strSql = " INSERT INTO det_rango_comi2 (emp_codigo,ran_com_codigo,det_com_ran_inferior,det_com_ran_superior,det_com_ran_porcentaje,det_com_ran_fechamod,det_com_ran_usumodmod)" & _
                 " VALUES('" & strEmpresa & "','" & lonCod & "','" & VSFG.TextMatrix(i, 1) & "','" & _
                 VSFG.TextMatrix(i, 2) & "','" & VSFG.TextMatrix(i, 3) & "',CURRENT_TIMESTAMP,substring_index(USER(),'@',1))"
        clsSql.Ejecutar strSql, "M"
    Next i
    If intAct = 1 Then
        strSql = " UPDATE rango_comi2 SET " & _
                 " ran_com_fechafin='" & DTPicker1.value & _
                 "', ran_com_activo='0',ran_com_fechamod=CURRENT_TIMESTAMP,ran_com_usumod=substring_index(USER(),'@',1)" & _
...
Private Sub Form_Activate()
    If Me.Tag = "M" And VSFG.Rows < 3 Then
        ...
        VSFG.TextMatrix(1, 3) = frmV_VerRangoComi2.VSFG.TextMatrix(1, 2)
    End If
End Sub
