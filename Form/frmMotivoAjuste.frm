...
Begin VB.Form frmMotivoAjuste 
   BackColor       =   &H00DDDDDD&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Motivo de Ajuste"
   ClientHeight    =   6960
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8280
   Icon            =   "frmMotivoAjuste.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmMotivoAjuste.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmMotivoAjuste"
...
Private Sub IniDato()
    Tipo = "Motivo de Ajuste"
    Tipo2 = "Motivo de Ajuste"
    Me.Caption = Tipo
End Sub

Private Sub cmdCambiar_Click()
    frmCambiarColeccion.Show
End Sub
...
End Sub
Private Sub Carga()
   strSql = " SELECT mot_aju_codigo,mot_aju_nombre,mot_aju_fechamod, mot_aju_usumod, '0' as modi  " & _
            " FROM motivo_ajuste " & _
            " WHERE emp_codigo='" & strEmpresa & "' "
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND mot_aju_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND mot_aju_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY mot_aju_codigo "
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
           strSql = " UPDATE motivo_ajuste " & _
                    " SET mot_aju_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                    " mot_aju_fechamod=CURRENT_TIMESTAMP,mot_aju_usumod='" & strUsuario & "' " & _
                    " WHERE mot_aju_codigo='" & VSFG.TextMatrix(i, 1) & "' AND emp_codigo='" & strEmpresa & "'"
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                ...
                control = 1
            Else
                strSql = " SELECT mot_aju_codigo " & _
                    " FROM motivo_ajuste " & _
                    " WHERE mot_aju_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                    " AND emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar strSql
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                   strSql = " INSERT INTO motivo_ajuste(mot_aju_codigo,emp_codigo,mot_aju_nombre,mot_aju_fechamod,mot_aju_usumod) " & _
