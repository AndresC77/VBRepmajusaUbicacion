...
Begin VB.Form frmDocPago 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CheckBox chkFiltroNombre 
         BackColor       =   &H00DDDDDD&
         Caption         =   "Filtrar Nombre Documento"
         BeginProperty Font 
            Name            =   "Arial"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmDocPago.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Private Sub Carga()
    ...
    strSql = " SELECT tip_doc_pag_codigo, tip_doc_pag_nombre, tip_doc_pag_descripcion, " & _
             " CONCAT(ban_codigo,'@@',cta_ban_numero),tip_doc_pag_fechamod, tip_doc_pag_usumod, '0' as modi" & _
             " FROM tipo_doc_pago " & _
             " WHERE tip_doc_pag_codigo LIKE '%'"
  
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  tip_doc_pag_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  tip_doc_pag_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY tip_doc_pag_nombre "
    clsCon_Def.Ejecutar strSql
    Set VSFG.DataSource = clsCon_Def.adorec_Def.DataSource
    ucrtVSFG.PonerNum
    'crea combo
    strSql = " SELECT CONCAT(cta_banco.ban_codigo,'@@',cta_ban_numero) as cta_ban_numero, CONCAT(ban_nombre,' - ',cta_ban_numero) as cta_ban_nombre " & _
             " FROM cta_banco INNER JOIN banco ON cta_banco.ban_codigo=banco.ban_codigo " & _
             " WHERE cta_banco.emp_codigo='" & strEmpresa & "' " & _
             " ORDER BY CONCAT(ban_nombre,' - ',cta_ban_numero) "
    clsCon_Def.Ejecutar strSql
    VSFG.ColComboList(4) = VSFG.BuildComboList(clsCon_Def.adorec_Def, " *cta_ban_nombre", "cta_ban_numero")
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        If VSFG.TextMatrix(i, 4) <> "" Then
            strBan = "'" & Left(VSFG.TextMatrix(i, 4), InStr(1, VSFG.TextMatrix(i, 4), "@@") - 1) & "'"
            strCta = "'" & Right(VSFG.TextMatrix(i, 4), Len(VSFG.TextMatrix(i, 4)) - InStr(1, VSFG.TextMatrix(i, 4), "@@") - 1) & "'"
        Else
            strBan = "NULL"
            strCta = "NULL"
        End If
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
             strSql = " UPDATE tipo_doc_pago " & _
                        " SET  tip_doc_pag_nombre = '" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                        " tip_doc_pag_descripcion = '" & UCase(VSFG.TextMatrix(i, 3)) & "'," & _
                        " ban_codigo = " & strBan & "," & _
                        " cta_ban_numero = " & strCta & "," & _
                        " tip_doc_pag_fechamod=CURRENT_TIMESTAMP, " & _
                        " tip_doc_pag_usumod='" & strUsuario & "' " & _
                        " WHERE tip_doc_pag_codigo='" & VSFG.TextMatrix(i, 1) & "' "
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                ...
                control = 1
            Else
                strSql = " SELECT tip_doc_pag_codigo " & _
                         " FROM tipo_doc_pago " & _
                         " WHERE tip_doc_pag_codigo='" & VSFG.TextMatrix(i, 1) & "' "
                clsCon_Def.Ejecutar strSql
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    strSql = " INSERT INTO tipo_doc_pago" & _
                             " (tip_doc_pag_codigo,tip_doc_pag_nombre, tip_doc_pag_descripcion," & _
                             " ban_codigo,cta_ban_numero,tip_doc_pag_fechamod, tip_doc_pag_usumod) " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            ...
            strSql = " SELECT count(tip_doc_pag_codigo) as existe " & _
                     " FROM doc_pago " & _
                     " WHERE tip_doc_pag_codigo='" & VSFG.TextMatrix(i, 1) & "' "
            clsCon_Def.Ejecutar (strSql)
