...
Begin VB.Form frmCourier 
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmCourier.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
End Sub
Private Sub Carga()
   strSQL = " SELECT cou_codigo,cou_nombre,cou_razon_social,cou_ruc,cou_ruta,cou_direccion," & _
            " cou_prefijo_secuencial,cou_secuencial_mascara,cou_secuencial_desde,cou_secuencial_hasta, " & _
            " cou_secuencial_actual,'' as vehiculo,cou_fechamod,cou_usumod, '0' as modi  " & _
            " FROM courier " & _
            " WHERE emp_codigo='" & strEmpresa & "' "
    If chkFiltroCodigo.Value = 1 Then
        strSQL = strSQL & "AND cou_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.Value = 1 Then
        strSQL = strSQL & " AND cou_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSQL = strSQL & " ORDER BY cou_nombre "
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
           strSQL = " UPDATE courier " & _
                    " SET cou_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                    " cou_razon_social='" & UCase(VSFG.TextMatrix(i, 3)) & "'," & _
                    " cou_ruc='" & UCase(VSFG.TextMatrix(i, 4)) & "'," & _
                    " cou_ruta='" & UCase(VSFG.TextMatrix(i, 5)) & "'," & _
                    " cou_direccion='" & UCase(VSFG.TextMatrix(i, 6)) & "'," & _
                    " cou_prefijo_secuencial='" & VSFG.TextMatrix(i, 7) & "'," & _
                    " cou_secuencial_mascara='" & VSFG.TextMatrix(i, 8) & "'," & _
                    " cou_secuencial_desde='" & VSFG.TextMatrix(i, 9) & "'," & _
                    " cou_secuencial_hasta='" & VSFG.TextMatrix(i, 10) & "'," & _
                    " cou_secuencial_actual='" & VSFG.TextMatrix(i, 11) & "'," & _
                    " cou_fechamod=CURRENT_TIMESTAMP,cou_usumod='" & strUsuario & "' " & _
                    " WHERE cou_codigo='" & VSFG.TextMatrix(i, 1) & "' AND emp_codigo='" & strEmpresa & "'"
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                ...
                control = 1
            Else
                strSQL = " SELECT cou_codigo " & _
                    " FROM courier " & _
                    " WHERE cou_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                    " AND emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar strSQL
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                   strSQL = " INSERT INTO courier(emp_codigo,cou_codigo,cou_nombre,cou_razon_social,cou_ruc,cou_ruta,cou_direccion," & _
                            " cou_prefijo_secuencial,cou_secuencial_mascara,cou_secuencial_desde,cou_secuencial_hasta, " & _
                            " cou_secuencial_actual,cou_fechamod,cou_usumod) " & _
                            " VALUES ('" & strEmpresa & "','" & UCase(VSFG.TextMatrix(i, 1)) & "','" & UCase(VSFG.TextMatrix(i, 2)) & "','" & UCase(VSFG.TextMatrix(i, 3)) & "','" & UCase(VSFG.TextMatrix(i, 4)) & "','" & UCase(VSFG.TextMatrix(i, 5)) & "','" & UCase(VSFG.TextMatrix(i, 6)) & "'," & _
                            " '" & VSFG.TextMatrix(i, 7) & "','" & VSFG.TextMatrix(i, 8) & "','" & VSFG.TextMatrix(i, 9) & "','" & VSFG.TextMatrix(i, 10) & "', " & _
                            " '" & VSFG.TextMatrix(i, 11) & "',CURRENT_TIMESTAMP, '" & strUsuario & "')"
                    clsCon_Def.Ejecutar strSQL, "M"
                Else
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            strSQL = " SELECT count(con_codigo) As existe " & _
                " FROM contenedor " & _
                " WHERE cou_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                " AND emp_codigo='" & strEmpresa & "'"
            clsCon_Def.Ejecutar (strSQL)
...
Private Sub VSFG_CellButtonClick(ByVal Row As Long, ByVal Col As Long)
    If Col = 12 Then
        frmCourierVehiculo.CodCou = VSFG.TextMatrix(Row, 1)
        frmCourierVehiculo.Show
