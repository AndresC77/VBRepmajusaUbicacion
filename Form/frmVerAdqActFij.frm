...
Begin VB.Form frmVerAdqActFij 
   BackColor       =   &H00DDDDDD&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Ver Adquisión Activos Fijos"
   ClientHeight    =   7425
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   7650
   Icon            =   "frmVerAdqActFij.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.Frame Frame4 
         ...
         Begin VSFlex8Ctl.VSFlexGrid VSFDetalle 
            ...
            FormatString    =   $"frmVerAdqActFij.frx":030A
            ScrollTrack     =   0   'False
            ScrollBars      =   3
            ScrollTips      =   0   'False
            MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmVerAdqActFij"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'################################################################################'
'#  Forma para visualizar los Activos Fijos realizados por concepto de          #
'#  Adquisiciones,  esta forma es solo de visualización, no permite la edición. #
'#  frmVerAdqActFij V1.0                                                        #
'#  Copyright (C) 2002                                                          #
'#                                                                              #
'#  Ventana para consultar los ingresos de Activos Fijos a una determinada  emp-#
...
'#    activo_fijo : En esta tabla se consulta el codigo,nombre y descripcion    #
'#                       del activo fijo.                                       #
'#    det_adquisicion_af : En esta tabla se consulta el detalle de adquisicion de   #
'#                        activos fijos.                                        #
'#                                                                              #
...
Private Sub CalcuTotal()
   'Calcula totales
    ...
    Next i
    TxtSubTotal = Format(SubTotal, "##0.00")
...
Private Sub dcmbAdq_codigo_Change()
    ...
    If clsConsu.adorec_Def.EOF = False Then
        ...
        Call limpiarFxGD
        'llenar flexgrid
        strSql = " SELECT det_adquisicion_af.act_fij_codigo, act_fij_nombre, act_fij_descripcion, act_fij_valor  " & _
                 " FROM det_adquisicion_af INNER JOIN activo_fijo ON  det_adquisicion_af.emp_codigo = activo_fijo.emp_codigo" & _
                 "                                              AND det_adquisicion_af.act_fij_codigo = activo_fijo.act_fij_codigo " & _
                 " WHERE det_adquisicion_af.emp_codigo = '" & strEmpresa & "' and det_adquisicion_af.adq_codigo =  " & dcmbAdq_codigo.Text & " "
...
'            " where  a.per_codigo = b.per_codigo and tip_adq_codigo  = 'ICA' and a.emp_codigo = '" & strEmpresa & "'" &
    ...
    If (clsConsu.adorec_Def.RecordCount = 0) Then
        MsgBox "No existe Adquision de Activos Fijos almacenados en el Sistema", vbInformation, "Sis-Admin"
        Exit Sub
    Else
        Set dcmbAdq_codigo.RowSource = clsConsu.adorec_Def.DataSource
        dcmbAdq_codigo.ListField = "adq_codigo"
