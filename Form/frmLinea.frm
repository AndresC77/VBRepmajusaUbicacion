...
Begin VB.Form frmLinea 
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmLinea.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
End Sub
Private Sub Carga()
   strSql = " SELECT lin_codigo,lin_nombre,lin_descripcion,lin_fechamod, lin_usumod, '0' as modi  " & _
            " FROM linea " & _
            " WHERE emp_codigo='" & strEmpresa & "' "
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  lin_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  lin_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY lin_nombre "
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
           strSql = " UPDATE linea " & _
                    " SET lin_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                    " lin_descripcion='" & UCase(VSFG.TextMatrix(i, 3)) & " '," & _
                    " lin_fechamod=CURRENT_TIMESTAMP,lin_usumod='" & strUsuario & "' " & _
                    " WHERE lin_codigo='" & VSFG.TextMatrix(i, 1) & "' AND emp_codigo='" & strEmpresa & "'"
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                ...
                control = 1
            Else
                strSql = " SELECT lin_codigo" & _
                    " FROM linea " & _
                    " WHERE lin_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                    " AND emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar strSql
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                   strSql = " INSERT INTO linea(lin_codigo,emp_codigo,lin_nombre,lin_descripcion,lin_fechamod,lin_usumod) " & _
