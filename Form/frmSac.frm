...
Begin VB.Form frmSac 
   ...
   Caption         =   "Servicio al Cliente"
   ClientHeight    =   6960
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10155
   Icon            =   "frmSac.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
...
Private Sub Carga()
    ...
    strSql = " SELECT sac_codigo, sac_nombre, sac_apellido, " & _
             " sac_fechamod, sac_usumod, '0' as modi" & _
             " From sac " & _
             " WHERE emp_codigo='" & strEmpresa & "' "
    
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  sac_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  (sac_apellido LIKE '%" & txtNombre.Text & "%' " & _
        " OR  sac_nombre LIKE '%" & txtNombre.Text & "%') "
    End If
    strSql = strSql & " ORDER BY sac_apellido,sac_nombre "
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            strSql = " UPDATE sac SET " & _
                 " sac_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "', " & _
                 " sac_apellido='" & UCase(VSFG.TextMatrix(i, 3)) & "', " & _
                 " sac_fechamod=CURRENT_TIMESTAMP, " & _
                 " sac_usumod='" & strUsuario & "' " & _
                 " WHERE sac_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            Else
                ...
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    strSql = " INSERT INTO sac " & _
                             " (sac_codigo, emp_codigo, sac_nombre, sac_apellido," & _
                             " sac_fechamod, sac_usumod ) " & _
