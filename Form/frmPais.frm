...
End Sub
Private Sub Carga()
    strSql = " SELECT pai_codigo,pai_nombre,pai_fechamod, pai_usumod, '0' as modi " & _
             " FROM pais " & _
             " WHERE pai_codigo LIKE '%'"
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  pai_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  pai_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY pai_nombre "
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            strSql = " UPDATE pais " & _
                 " SET pai_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                 " pai_fechamod=CURRENT_TIMESTAMP," & _
                 " pai_usumod='" & strUsuario & "' " & _
                 " WHERE pai_codigo='" & VSFG.TextMatrix(i, 1) & "'"
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                ...
                control = 1
            Else
                strSql = " SELECT pai_codigo" & _
                    " FROM pais " & _
                    " WHERE pai_codigo='" & VSFG.TextMatrix(i, 1) & "'"
                clsCon_Def.Ejecutar strSql
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    strSql = " INSERT INTO pais(pai_codigo,pai_nombre,pai_fechamod,pai_usumod) " & _
