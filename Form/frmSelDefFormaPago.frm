...
'################################################################################'
'#  Forma para la seleccion de la Línea y poder modificar,                      #
   ...
   Dim strSql As String
    ...
    Else
        ...
        If clsCon_Def.adorec_Def("Egr") > 0 Then
            MsgBox "No Puede eliminar esta Forma de Pago", vbInformation, "Eliminación"
        Else ' Si no existen ingresos ni egresos se puede eliminar esta forma de pago
            strSql = " DELETE " & _
                     ...
                     " AND emp_codigo='" & strEmpresa & "'"
            clsCon_Def.Ejecutar (strSql)
            MsgBox "Forma de Pago eliminada", vbInformation, "Eliminación"
        End If
    End If
...
' conocer que se esta modificando y ademas se envia el código de la linea que se modificará
...
' conocer que se esta ingresará una nueva linea
    frmDefFormaPago.Show
