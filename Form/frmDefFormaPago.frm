...
Begin VB.Form frmDefFormaPago 
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmDefFormaPago.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmDefFormaPago"
...
End Sub
Private Sub Carga()
    strSql = " SELECT for_pag_codigo,for_pag_nombre,for_pag_descripcion," & _
             " for_pag_tiempo,for_pag_periodo, " & _
             " for_pag_fechamod, for_pag_usumod, '0' as modi" & _
             " FROM forma_pago " & _
             " WHERE emp_codigo='" & strEmpresa & "' "
    clsCon_Def.Ejecutar (strSql)
    
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  for_pag_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  for_pag_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY for_pag_nombre "
    clsCon_Def.Ejecutar strSql
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        strSql = " UPDATE forma_pago " & _
                 " SET for_pag_nombre='" & (VSFG.TextMatrix(i, 2)) & "'," & _
                 " for_pag_descripcion='" & UCase(VSFG.TextMatrix(i, 3)) & "', " & _
                 " for_pag_tiempo=" & VSFG.TextMatrix(i, 4) & "," & _
                 " for_pag_periodo=" & VSFG.TextMatrix(i, 5) & " ," & _
                 " for_pag_fechamod=CURRENT_TIMESTAMP," & _
                 " for_pag_usumod='" & strUsuario & "' " & _
                 " WHERE for_pag_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                ...
                control = 1
            Else
                strSql = " SELECT for_pag_codigo" & _
                    " FROM forma_pago " & _
                    " WHERE for_pag_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                    " AND emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar strSql
                ...
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    strSql = " INSERT INTO forma_pago" & _
                             " (for_pag_codigo,emp_codigo,for_pag_nombre,for_pag_descripcion," & _
                             " for_pag_tiempo,for_pag_periodo,for_pag_fechamod,for_pag_usumod) " & _
                             " VALUES ('" & UCase(VSFG.TextMatrix(i, 1)) & "','" & strEmpresa & "'," & _
                             " '" & (VSFG.TextMatrix(i, 2)) & "','" & UCase(VSFG.TextMatrix(i, 3)) & "', " & _
                             "  " & VSFG.TextMatrix(i, 4) & " , " & VSFG.TextMatrix(i, 5) & " , " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            ...
            If clsCon_Def.adorec_Def("existe") > 0 Then
                ...
                strSql = " SELECT count(egr_codigo) As existe2 " & _
                         " FROM egreso " & _
                         " WHERE for_pag_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                         " AND emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar (strSql)
