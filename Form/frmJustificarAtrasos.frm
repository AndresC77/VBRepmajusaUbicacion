...
Begin VB.Form frmJustificarAtrasos 
   BackColor       =   &H00DDDDDD&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Justificación Atrasos"
   ClientHeight    =   8970
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   10770
   Icon            =   "frmJustificarAtrasos.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Frame fraEmpleado 
      ...
      Begin VB.Frame fraFecha 
         ...
         Begin VB.ComboBox cmbMesI 
            Height          =   315
            ItemData        =   "frmJustificarAtrasos.frx":030A
            Left            =   1320
            List            =   "frmJustificarAtrasos.frx":0335
      ...
      Begin VB.CommandButton cmdMostrar 
         ...
         Width           =   3255
      End
      Begin VB.CheckBox chkAtraso 
         BackColor       =   &H00DDDDDD&
         Caption         =   "Mostrar sin Atrasos"
         BeginProperty Font 
            Name            =   "Arial"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VB.Frame fraDetalle 
      ...
      Begin VSFlex8Ctl.VSFlexGrid VSFG 
         ...
         FormatString    =   $"frmJustificarAtrasos.frx":039E
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   1
   ...
   End
End
Attribute VB_Name = "frmJustificarAtrasos"
    ...
    If chkEntrada.value = 1 Then
        If chkSalida.value = 1 Then
            ...
            If dtpEntrada.value >= dtpSalida.value Then
                MsgBox "Defina correctamente la hora de salida de la visita", vbInformation, "Justificación de Visitas"
                Exit Sub
            End If
            ...
            strSql = " SELECT det_hor_dia,TIME_FORMAT(det_hor_entrada,'%H:%i:%s'),TIME_FORMAT(det_hor_salida,'%H:%i:%s'),TIME_FORMAT(det_hor_salida+COALESCE(hor_tol_salida_max,'00:00:00'),'%H:%i:%s'),TIME_FORMAT(det_hor_entrada+COALESCE(hor_tol_entrada_max,'00:00:00'),'%H:%i:%s') " & _
                     ...
                     " AND '" & Format(dtpEntrada.value, "HH:mm:SS") & "' BETWEEN TIME_FORMAT(det_hor_entrada-COALESCE(hor_tol_entrada_min,'00:00:00'),'%H:%i:%s') AND TIME_FORMAT(det_hor_salida+COALESCE(hor_tol_salida_max,'00:00:00'),'%H:%i:%s') " & _
            ...
            If clsSql.adorec_Def.RecordCount = 0 Then
                MsgBox "No se encontró un horario para la fecha/hora definida", vbInformation, "Justificación Atrasos"
                Exit Sub
            End If
            ...
            Atraso = Format(diferenciaHoraria(entTol, Format(dtpEntrada.value, "hh:mm:ss")), "hh:mm:ss")
            ...
            strSql = " SELECT COUNT(*) " & _
                     ...
                     " AND ast_hora='" & horS & "' " & _
                     " AND ast_dia='" & DatePart("w", dtpFecha.value, vbMonday) - 1 & "' "
            clsSql.Ejecutar strSql
            If clsSql.adorec_Def.RecordCount > 0 Then
                If FormatoD0(clsSql.adorec_Def(0)) > 0 Then
                    MsgBox "Existe un registro de asistencia en la fecha/hora definida", vbInformation, "Justificación Atrasos"
                    Exit Sub
                End If
            End If
            
            Atrasado = 0
            If Atraso <> "00:00:00" Then
                Atrasado = 1
            End If
            
            Registrado = 0
            If Format(dtpSalida.value, "hh:mm:ss") <> "00:00:00" Then
                Registrado = 1
            End If
            
            Multa = 0
            If Atrasado = 1 Then
                strSql = " SELECT COALESCE(det_mul_valor,'0') as valor " & _
                         " FROM det_multas " & _
                         " WHERE emp_codigo='" & strEmpresa & "' " & _
                         " AND mul_codigo='1' " & _
                         " AND '" & Format(Atraso, "HH:MM:SS") & "' BETWEEN det_mul_inferior AND det_mul_superior "
                clsSql.Ejecutar strSql
                If clsSql.adorec_Def.RecordCount > 0 Then
                    Multa = FormatoD4(clsSql.adorec_Def(0))
                End If
            End If
            
            strSql = " INSERT INTO asistencia(emp_codigo,epl_codigo, ast_fecha, ast_dia, ast_hora, " & _
                     " ast_entrada_esp,ast_entrada, ast_salida_esp, ast_atrasado, ast_tiempo_atraso,  ast_fechamod,ast_usumod,ast_salida_tol,ast_multa,ast_observacion," & _
                     " ast_registrado,ast_salida) " & _
                     " VALUES ('" & strEmpresa & "','" & cmbEmpleado.BoundText & "', '" & Format(dtpFecha.value, "yyyy-MM-dd") & "','" & DatePart("w", dtpFecha.value, vbMonday) - 1 & "', '" & horS & "','" & _
                     horE & "','" & Format(dtpEntrada.value, "hh:mm:ss") & "','" & horS & "','" & Atrasado & "','" & Atraso & "',CURRENT_TIMESTAMP,'" & strUsuario & "','" & salTol & "','" & Multa & "','" & Observacion & "','" & _
                     Registrado & "','" & Format(dtpSalida.value, "hh:mm:ss") & "') "
        ...
        Else
            ...
            strSql = " INSERT INTO asistencia(emp_codigo,epl_codigo, ast_fecha, ast_dia, ast_hora, " & _
                     " ast_entrada_esp,ast_entrada, ast_salida_esp, ast_atrasado, ast_tiempo_atraso,  ast_fechamod,ast_usumod,ast_salida_tol,ast_multa,ast_observacion) " & _
                     " VALUES ('" & strEmpresa & "','" & cmbEmpleado.BoundText & "', '" & Format(dtpFecha.value, "yyyy-MM-dd") & "','" & DatePart("w", dtpFecha.value, vbMonday) - 1 & "', '" & horS & "','" & _
                     horE & "','" & Format(dtpEntrada.value, "hh:mm:ss") & "','" & horS & "','" & Atrasado & "','" & Atraso & "',CURRENT_TIMESTAMP,'" & strUsuario & "','" & salTol & "','" & Multa & "','" & Observacion & "') "
            clsSql.Ejecutar strSql, "M"
        End If
    Else
        strSql = " SELECT det_hor_dia,TIME_FORMAT(det_hor_entrada,'%H:%i:%s'),TIME_FORMAT(det_hor_salida,'%H:%i:%s'),TIME_FORMAT(det_hor_salida+COALESCE(hor_tol_salida_max,'00:00:00'),'%H:%i:%s'),TIME_FORMAT(det_hor_entrada+COALESCE(hor_tol_entrada_max,'00:00:00'),'%H:%i:%s') " & _
        ...
        If clsSql.adorec_Def.RecordCount = 0 Then
            MsgBox "No se encontró un horario para la fecha/hora defina", vbInformation, "Justificación Atrasos"
            Exit Sub
        End If
        ...
        strSql = " SELECT COUNT(*) " & _
                 ...
                 " AND ast_salida_esp='" & horS & "' " & _
                 " AND (ast_salida IS NULL or ast_salida='00:00:00')"
        clsSql.Ejecutar strSql
        If clsSql.adorec_Def.RecordCount > 0 Then
            If FormatoD0(clsSql.adorec_Def(0)) = 0 Then
                MsgBox "No existe un registro de asistencia en la fecha/hora definida", vbInformation, "Justificación Atrasos"
                Exit Sub
            End If
        End If
        ...
        strSql = "UPDATE asistencia SET " & _
                " ast_salida = '" & Format(dtpSalida.value, "hh:mm:ss") & "'," & _
                " ast_registrado='" & Registrado & "'," & _
                " ast_observacion='" & Observacion & "', " & _
...
Private Sub Cargar()
    Limpiar
    strSql = " SELECT DATE_FORMAT(ast_fecha,'%w'), " & _
             " ast_fecha,TIME_FORMAT(ast_entrada_esp,'%H:%i:%s'),TIME_FORMAT(ast_entrada,'%H:%i:%s')," & _
             " TIME_FORMAT(ast_salida_esp,'%H:%i:%s'),TIME_FORMAT(ast_salida,'%H:%i:%s') " & _
             " FROM asistencia " & _
    ...
    If chkAtraso.value = 1 Then
        strSql = strSql & " AND ast_atrasado='0' "
    End If
    ...
    strSql = strSql & " ORDER BY ast_fecha,ast_entrada_esp,ast_entrada,ast_salida_esp,ast_salida "
    clsSql.Ejecutar strSql
