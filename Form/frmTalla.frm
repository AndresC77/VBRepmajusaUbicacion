...
Begin VB.Form frmTalla 
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmTalla.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
End Sub
Private Sub Carga()
   strSql = " SELECT tal_codigo,tal_nombre,tal_descripcion,tal_fechamod, tal_usumod, '0' as modi  " & _
            " FROM talla " & _
            " WHERE emp_codigo='" & strEmpresa & "' "
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND tal_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND tal_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY tal_nombre "
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
           strSql = " UPDATE talla " & _
                    " SET tal_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                    " tal_descripcion='" & UCase(VSFG.TextMatrix(i, 3)) & " '," & _
                    " tal_fechamod=CURRENT_TIMESTAMP,tal_usumod='" & strUsuario & "' " & _
                    " WHERE tal_codigo='" & VSFG.TextMatrix(i, 1) & "' AND emp_codigo='" & strEmpresa & "'"
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                ...
                control = 1
            Else
                strSql = " SELECT tal_codigo" & _
                    " FROM talla " & _
                    " WHERE tal_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                    " AND emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar strSql
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                   strSql = " INSERT INTO talla(tal_codigo,emp_codigo,tal_nombre,tal_descripcion,tal_fechamod,tal_usumod) " & _
                            " VALUES ('" & UCase(VSFG.TextMatrix(i, 1)) & "','" & strEmpresa & "','" & UCase(VSFG.TextMatrix(i, 2)) & "','" & UCase(VSFG.TextMatrix(i, 3)) & "', " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            strSql = " SELECT count(prd_codigo) As existe " & _
                " FROM producto " & _
                " WHERE tal_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                " AND emp_codigo='" & strEmpresa & "'"
            clsCon_Def.Ejecutar (strSql)
