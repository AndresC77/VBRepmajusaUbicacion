...
Begin VB.Form frmMovEgreso 
   Appearance      =   0  'Flat
   BackColor       =   &H00DDDDDD&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Definición de Cta. Contable para Movimientos Egreso"
   ClientHeight    =   3945
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   3465
   ...
   End
End
Attribute VB_Name = "frmMovEgreso"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'################################################################################'
'#  Forma de modificación de la Cta.Contable de los movimientos de egreso de    #
'#  mercaderia y del IVA.                                                       #
'#  frmMovEgreso V1.0                                                           #
'#  Copyright (C) 2002                                                          #
'#                                                                              #
'#  Ventana para la modificación de los datos de los movimientos de egreso      #
'#  de mercadería.                                                              #
'#                                                                              #
'#  Tablas que se maneja:                                                       #
'#  tipo_egreso :En esta tabla se almacenan los tipos de egresos y se           #
...
Private Sub cmbAceptar_Click()
    ...
    ' Si se esta modificando el Movimiento
    ElseIf Me.Tag = "M" Then
    'Almacenamiento de los cambios realizados al movimiento
        strSql = " UPDATE tipo_egreso " & _
                 " SET tip_egr_ctaconta='" & dcmbCtaConta.Text & "',tip_egr_impuesto=" & chkIVA.value & _
                 ", tip_egr_ctaconta2='" & dcmbCtaConta2.Text & "' " & _
                 ",tip_egr_fechamod=CURRENT_TIMESTAMP,tip_egr_usumod='" & strUsuario & "' " & _
                 " WHERE tip_egr_codigo='" & txtCodigo.Text & "' AND emp_codigo='" & strEmpresa & "'"
    End If
...
Private Sub Form_Activate()
    ...
    ElseIf Me.Tag = "N" Then
        ...
        txtCodigo.Enabled = False
        'Consulta el codigo de la lisla que tocaría ingresar (autonumerico)
