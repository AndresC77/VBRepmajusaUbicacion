...
End Sub
Private Sub Carga()
    strSql = " SELECT are_codigo,are_nombre,are_ctaconta,are_descripcion,are_fechamod,are_usumod, '0' as modi " & _
             " FROM area " & _
             " WHERE emp_codigo = '" & strEmpresa & "'"
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  are_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  are_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY are_codigo,are_nombre "
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            If Trim(VSFG.TextMatrix(i, 2)) = "" Then
                ...
                control = 1
            Else
           strSql = " UPDATE area " & _
                 " SET are_nombre='" & UCase(Trim(VSFG.TextMatrix(i, 2))) & "'," & _
                 " are_descripcion='" & UCase(Trim(VSFG.TextMatrix(i, 4))) & "'," & _
                 " are_ctaconta='" & VSFG.TextMatrix(i, 3) & "'," & _
                 " are_fechamod=CURRENT_TIMESTAMP," & _
                 " are_usumod='" & strUsuario & "' " & _
                 " WHERE are_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            Else
                ...
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    ...
                    strSql = " INSERT INTO area(are_codigo,emp_codigo,are_nombre,are_ctaconta,are_descripcion,are_fechamod,are_usumod) " & _
                            " VALUES ('" & UCase(Trim(VSFG.TextMatrix(i, 1))) & "','" & strEmpresa & "','" & UCase(Trim(VSFG.TextMatrix(i, 2))) & "', " & _
                            "'" & VSFG.TextMatrix(i, 3) & "','" & UCase(Trim(VSFG.TextMatrix(i, 4))) & "', " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            ...
            strSql = " SELECT count(*) As existe " & _
                ...
                " AND emp_codigo='" & strEmpresa & "'"
            clsCon_Def.Ejecutar (strSql)
            cont = cont + Val(clsCon_Def.adorec_Def("existe"))
            strSql = " SELECT count(*) As existe " & _
                " FROM departamento " & _
                " WHERE are_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                " AND emp_codigo='" & strEmpresa & "'"
