...
'if para ver q los campos esten llenos
   ...
   If Len(txtCostoServicio.Text) = 0 Then
    MsgBox ("EL CAMPO Costo Servicio ESTA VACIO")
   ...
   End If
    ...
    If VSFG.TextMatrix(1, 1) = "" Or VSFG.TextMatrix(1, 2) = "" Or FormatoD4(VSFG.TextMatrix(1, 3)) <= 0 Then
      MsgBox ("Debe ingresar datos en el detalle...."), vbInformation
       Exit Sub
    Else

        strSql = " INSERT INTO preproducto_ficha(emp_codigo,pre_codigo,pre_fic_version,pre_fic_costo_servicios,pre_fic_observacion,pre_fic_fechamod,pre_fic_usumod) " & _
                 " VALUES ( '" & strEmpresa & "' , '" & txtReferencia.Text & "' , '" & txtVersion.Text & "' " & _
                 ", '" & txtCostoServicio.Text & "' , '" & txtObservacion.Text & "'," & _
        ...
        clsCon_Def.Ejecutar strSql, "M"
         ...
         If VSFG.TextMatrix(i, 1) <> "" And VSFG.TextMatrix(i, 2) <> "" And FormatoD4(VSFG.TextMatrix(i, 3)) > 0 Then
                      'insert en det_preproducto_ficha
                    strSql = " INSERT INTO det_preproducto_ficha(emp_codigo,pre_codigo,pre_fic_version,prd_codigo,det_pre_fic_cantidad,det_pre_fic_fechamod,det_pre_fic_usumod) " & _
                            " VALUES ('" & strEmpresa & "','" & txtReferencia.Text & "' , '" & txtVersion.Text & "','" & UCase(VSFG.TextMatrix(i, 1)) & "','" & FormatoD4(VSFG.TextMatrix(i, 3)) & "', " & _
                            " CURRENT_TIMESTAMP, '" & strUsuario & "')"
                    clsCon_Def.Ejecutar strSql, "M"
...
Private Sub VSFG_CellChanged(ByVal Row As Long, ByVal Col As Long)
    ...
    If VSFG.TextMatrix(Row, 1) <> "" And VSFG.TextMatrix(Row, 2) <> "" And VSFG.TextMatrix(Row, 3) <> "" Then
        VSFG.AddItem ""
    End If
    If VSFG.TextMatrix(1, 1) <> "" And VSFG.TextMatrix(1, 2) <> "" And FormatoD4(VSFG.TextMatrix(1, 3)) > 0 Then
