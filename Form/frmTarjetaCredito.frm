...
Begin VB.Form frmTarjetaCredito 
   BackColor       =   &H00DDDDDD&
   ...
   ClientTop       =   330
   ClientWidth     =   10155
   Icon            =   "frmTarjetaCredito.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmTarjetaCredito.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmTarjetaCredito"
...
Private Sub Carga()
    ...
    strSql = " SELECT tar_cre_codigo,tar_cre_nombre,tar_cre_descripcion,tip_com_codigo,tar_cre_porcentaje, " & _
             " tar_cre_fechamod, tar_cre_usumod, '0' as modi" & _
             " FROM tarjeta_credito " & _
             " WHERE emp_codigo='" & strEmpresa & "' "
             
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  tar_cre_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  tar_cre_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY tar_cre_nombre "
    clsCon_Def.Ejecutar strSql
    Set VSFG.DataSource = clsCon_Def.adorec_Def.DataSource
    ucrtVSFG.PonerNum
    
    strSql = " SELECT tip_com_codigo, tip_com_nombre " & _
             " FROM tipo_comision " & _
             " ORDER BY tip_com_codigo "
    clsCon_Def.Ejecutar strSql
    VSFG.ColComboList(4) = VSFG.BuildComboList(clsCon_Def.adorec_Def, " *tip_com_nombre", "tip_com_codigo")
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            strSql = " UPDATE tarjeta_credito " & _
                 " SET tar_cre_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                 " tar_cre_descripcion='" & UCase(VSFG.TextMatrix(i, 3)) & "', " & _
                 " tip_com_codigo= " & VSFG.TextMatrix(i, 4) & "," & _
                 " tar_cre_porcentaje= " & FormatoD2(VSFG.TextMatrix(i, 5)) & "," & _
                 " tar_cre_fechamod=CURRENT_TIMESTAMP," & _
                 " tar_cre_usumod='" & strUsuario & "' " & _
                 " WHERE tar_cre_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 4) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta tipo de comisión", vbInformation, "Ingreso"
                control = 1
            ElseIf VSFG.TextMatrix(i, 5) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta % cotización", vbInformation, "Ingreso"
                control = 1
            Else
                strSql = " SELECT tar_cre_codigo " & _
                         " FROM tarjeta_credito " & _
                         " WHERE " & _
                         " tar_cre_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
                         " AND emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar strSql
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                
                    strSql = " INSERT INTO tarjeta_credito" & _
                             " (tar_cre_codigo,emp_codigo,tar_cre_nombre,tar_cre_descripcion," & _
                             " tip_com_codigo,tar_cre_porcentaje,tar_cre_fechamod,tar_cre_usumod) " & _
                             " VALUES ('" & UCase(VSFG.TextMatrix(i, 1)) & "','" & strEmpresa & "'," & _
                             " '" & UCase(VSFG.TextMatrix(i, 2)) & "','" & UCase(VSFG.TextMatrix(i, 3)) & "'," & _
                             " '" & VSFG.TextMatrix(i, 4) & "','" & FormatoD2(VSFG.TextMatrix(i, 5)) & "', " & _
...
''                     " FROM egreso " & _
''                     " WHERE com_tc_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
''                     " AND emp_codigo='" & strEmpresa & "'"
''            clsCon_Def.Ejecutar (strSql)
