...
Begin VB.Form frmVendedor 
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmVendedor.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Private Sub Carga()
    ...
    strSql = " SELECT ven_codigo, ven_cedula, " & _
             " ven_nombre, ven_apellido, " & _
             " ven_cargo,ven_direccion, ven_telf, ven_telf2," & _
             " ven_email,ven_cod_empleado, ven_signatura,ven_comision," & _
             " ven_fechamod, ven_usumod, '0' as modi" & _
             " From vendedor " & _
             " WHERE emp_codigo='" & strEmpresa & "' "
    
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  ven_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  (ven_apellido LIKE '%" & txtNombre.Text & "%' " & _
        " OR  ven_nombre LIKE '%" & txtNombre.Text & "%') "
    End If
    strSql = strSql & " ORDER BY ven_apellido,ven_nombre "
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            strSql = " UPDATE vendedor SET " & _
                 " ven_cedula='" & UCase(VSFG.TextMatrix(i, 2)) & "', " & _
                 " ven_nombre='" & UCase(VSFG.TextMatrix(i, 3)) & "', " & _
                 " ven_apellido='" & UCase(VSFG.TextMatrix(i, 4)) & "', " & _
                 " ven_cargo='" & UCase(VSFG.TextMatrix(i, 5)) & "', " & _
                 " ven_direccion='" & UCase(VSFG.TextMatrix(i, 6)) & "', " & _
                 " ven_telf='" & Trim(VSFG.TextMatrix(i, 7)) & "', " & _
                 " ven_telf2='" & Trim(VSFG.TextMatrix(i, 8)) & "', " & _
                 " ven_email='" & VSFG.TextMatrix(i, 9) & "', " & _
                 " ven_cod_empleado='" & UCase(VSFG.TextMatrix(i, 10)) & "'," & _
                 " ven_signatura='" & UCase(VSFG.TextMatrix(i, 11)) & "', " & _
                 " ven_comision='" & FormatoD4(VSFG.TextMatrix(i, 12)) & "', " & _
                 " ven_fechamod=CURRENT_TIMESTAMP, " & _
                 " ven_usumod='" & strUsuario & "' " & _
                 " WHERE ven_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta cédula", vbInformation, "Ingreso"
                control = 1
            ...
            ElseIf VSFG.TextMatrix(i, 4) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta apellido", vbInformation, "Ingreso"
                control = 1
            'ElseIf VSFG.TextMatrix(i, 10) = "" Then
            '    MsgBox "No puede ingresar " & Tipo2 & " falta código de empleado", vbInformation, "Ingreso"
                'control = 1
            Else
                strSql = " SELECT ven_codigo " & _
                         " FROM vendedor " & _
                         " WHERE ven_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
                         " AND emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar strSql
                ...
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    strSql = " INSERT INTO vendedor " & _
                             " (ven_codigo, emp_codigo, ven_cedula, ven_nombre, ven_apellido," & _
                             " ven_cargo, ven_direccion, ven_telf, " & _
                             " ven_telf2, ven_email, ven_cod_empleado, ven_signatura,ven_comision, " & _
                             " ven_fechamod, ven_usumod ) " & _
                             " VALUES ('" & UCase(VSFG.TextMatrix(i, 1)) & "','" & strEmpresa & "','" & UCase(VSFG.TextMatrix(i, 2)) & "', " & _
                             " '" & UCase(VSFG.TextMatrix(i, 3)) & "','" & UCase(VSFG.TextMatrix(i, 4)) & "', " & _
                             " '" & UCase(VSFG.TextMatrix(i, 5)) & "','" & UCase(VSFG.TextMatrix(i, 6)) & "', " & _
                             " '" & Trim(VSFG.TextMatrix(i, 7)) & "','" & Trim(VSFG.TextMatrix(i, 8)) & "'," & _
                             " '" & (VSFG.TextMatrix(i, 9)) & "','" & (VSFG.TextMatrix(i, 10)) & "'," & _
                             " '" & UCase(VSFG.TextMatrix(i, 11)) & "','" & FormatoD4(VSFG.TextMatrix(i, 12)) & "', " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
             strSql = " SELECT count(pro_ven_codigo) as existe " & _
                     " FROM proyecto_venta " & _
                     " WHERE ven_codigo = '" & VSFG.TextMatrix(i, 1) & "' " & _
                     " AND emp_codigo='" & strEmpresa & "'"
            clsCon_Def.Ejecutar (strSql)
            ...
            Else
                strSql = " SELECT count(age_numero) as existe2 " & _
                         " FROM agenda " & _
                         " WHERE ven_codigo = '" & VSFG.TextMatrix(i, 1) & "' " & _
                         " AND emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar (strSql)
...
Private Sub VSFG_BeforeEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    ElseIf Val(VSFG.TextMatrix(Row, VSFG.Cols - 1)) = 3 Or Val(VSFG.TextMatrix(Row, VSFG.Cols - 1)) = -3 Then
        ...
        End If
    End If
    If VSFG.TextMatrix(Row, 12) = "" Then
        VSFG.TextMatrix(Row, 12) = "0"
    End If
End Sub
...
Private Sub VSFG_AfterEdit(ByVal Row As Long, ByVal Col As Long)
    ...
    End If
    If Col = 12 Then
        If VSFG.TextMatrix(Row, 12) <> "" And Not IsNumeric(VSFG.TextMatrix(Row, 12)) Then
            VSFG.TextMatrix(Row, 12) = "0"
        End If
    End If
End Sub
