...
Begin VB.Form frmClienteMod 
   ...
   ScaleHeight     =   7545
   ScaleWidth      =   12285
   Begin VB.CheckBox chkBloqueoDesBloqueo 
      BackColor       =   &H00DDDDDD&
   ...
   Begin MSDataListLib.DataCombo cmbNegocio 
      ...
      EndProperty
   End
   Begin VB.CommandButton cmdVerReferido 
      Caption         =   "Ver Referido"
      Height          =   375
      Left            =   8880
      TabIndex        =   31
      Top             =   2880
      ...
      Width           =   1575
   End
   Begin VB.CommandButton cmdReferido 
      Caption         =   "Cargar Referido"
      Height          =   375
      Left            =   10560
      TabIndex        =   30
      Top             =   2880
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmClienteMod.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   Begin VB.Image imgBtnUp 
      Height          =   240
      Left            =   5760
      Picture         =   "frmClienteMod.frx":0CB6
   ...
   End
End
Attribute VB_Name = "frmClienteMod"
...
Private Sub chkBloqueoDesBloqueo_Click()
    Dim i As Long
    'Mod = 0 NADA - 1 ELIMINAR - 2 INSERTAR - 3 MODIFICAR - -2 NADA INSERTAR - -3 NADA MODIF
    For i = 1 To VSFG.Rows - 1
        VSFG.ShowCell i, 54
        If Abs(VSFG.TextMatrix(i, VSFG.Cols - 1)) = 2 Or Abs(VSFG.TextMatrix(i, VSFG.Cols - 1)) = 3 And VSFG.TextMatrix(i, 54) <> chkBloqueoDesBloqueo.Value Then
            VSFG.TextMatrix(i, 54) = chkBloqueoDesBloqueo.Value
            VSFG.TextMatrix(i, VSFG.Cols - 1) = Abs(VSFG.TextMatrix(i, VSFG.Cols - 1))
        End If
    Next i
    If chkBloqueoDesBloqueo.Value = 1 Then
        chkBloqueoDesBloqueo.Caption = "DesBloquear Todos"
    Else
        chkBloqueoDesBloqueo.Caption = "Bloquear Todos"
    End If
...
Private Sub Carga(Optional booTodo As Boolean = True)
    ...
    strSql = " SELECT per_codigo,per_cm,per_rcm,'' as red, " & _
             ...
             " AND tip_ped_codigo LIKE '" & cmbNegocio.BoundText & "' "
    If chkFiltroCodigo.Value = 1 Then
        If InStr(1, txtCodigo.Text, vbNewLine) > 0 Then
            ReDim strRucs(UBound(Split(txtCodigo.Text, vbNewLine)))
            strRucs = Split(txtCodigo.Text, vbNewLine)
            strSql = strSql & "AND (per_ruc like 'qqq'"
            For i = 0 To UBound(strRucs)
                If Trim(strRucs(i)) <> "" Then
                    strSql = strSql & " OR per_ruc like '%" & Trim(strRucs(i)) & "'"
                End If
            Next i
            strSql = strSql & ")"
        Else
    ...
    'If booTodo = True Then
        ...
        VSFG.ColComboList(15) = VSFG.BuildComboList(clsACombo2.adorec_Def, "*ciucanpai", "ciu_codigo")
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            strSql = " SELECT tip_ped_cliente_unico FROM tipo_pedido " & _
                     " WHERE emp_codigo='" & strEmpresa & "' " & _
                     " AND tip_ped_codigo='" & VSFG.TextMatrix(i, 39) & "' "
            clsCon_Def.Ejecutar strSql
            If FormatoD0(clsCon_Def.adorec_Def("tip_ped_cliente_unico")) = 0 Then
                FiltroPedido = "'" & VSFG.TextMatrix(i, 28) & "'"
            Else
                FiltroPedido = " SELECT tip_ped_codigo FROM tipo_pedido WHERE emp_codigo='" & strEmpresa & "' AND tip_ped_cliente_unico=1"
            End If
            strSql = " SELECT count(per_ruc) " & _
                     " FROM persona " & _
                     " WHERE emp_codigo='" & strEmpresa & "' " & _
                     " AND cat_p_tipo='C' " & _
                     " AND per_ruc='" & UCase(Trim(VSFG.TextMatrix(i, 14))) & "' " & _
                     " AND tip_ped_codigo IN ( " & FiltroPedido & " ) " & _
                     " " & _
            ...
            If entra = 0 Then
                If TIPOUsu = "JSA" Then
                    ...
                    strSqlAux = " UPDATE persona " & _
                             " SET per_observacion_mod = CONCAT(COALESCE(per_observacion_mod,'')," & _
                             " IIF(per_dcto!='" & FormatoD2(VSFG.TextMatrix(i, 29)) & "'," & _
                    ...
                    strSql = strSql & " zon_codigo='" & UCase(VSFG.TextMatrix(i, 18)) & "'," & _
                         " per_direccion='" & UCase(VSFG.TextMatrix(i, 19)) & "'," & _
                         " per_ubicacion='" & UCase(VSFG.TextMatrix(i, 20)) & "'," & _
                         " per_telf='" & UCase(VSFG.TextMatrix(i, 21)) & "'," & _
                         " per_fax='" & UCase(VSFG.TextMatrix(i, 22)) & "'," & _
                         " per_celular='" & UCase(VSFG.TextMatrix(i, 23)) & "'," & _
                         " per_email='" & VSFG.TextMatrix(i, 24) & "'," & _
                         " per_fechacumplea='" & VSFG.TextMatrix(i, 25) & "'," & _
                         " per_direccion2='" & UCase(VSFG.TextMatrix(i, 26)) & "'," & _
                         " for_ent_codigo='" & VSFG.TextMatrix(i, 27) & "'," & _
                         " per_dcto='" & FormatoD4(VSFG.TextMatrix(i, 29)) & "',"
                    ...
                    strSql = strSql & " sac_codigo='" & VSFG.TextMatrix(i, 70) & "'," & _
                         ...
                         " per_aplica_ret='" & Abs(FormatoD0(VSFG.TextMatrix(i, 73))) & "',"
                ElseIf TIPOUsu = "SAC" Then
                    strSql = " UPDATE persona " & _
                         " SET per_cm='" & Abs(FormatoD0(VSFG.TextMatrix(i, 2))) & "'," & _
                         " per_rcm='" & Abs(FormatoD0(VSFG.TextMatrix(i, 3))) & "'," & _
                         " per_tipo='" & VSFG.TextMatrix(i, 5) & "'," & _
                    ...
                    strSql = strSql & " zon_codigo='" & UCase(VSFG.TextMatrix(i, 18)) & "'," & _
                         ...
                         " per_fechacumplea='" & VSFG.TextMatrix(i, 25) & "'," & _
                         " per_direccion2='" & UCase(VSFG.TextMatrix(i, 26)) & "'," & _
                         " for_ent_codigo='" & VSFG.TextMatrix(i, 27) & "',"
                    strSql = strSql & " ven_codigo='" & VSFG.TextMatrix(i, 38) & "'," & _
                         " tip_ped_codigo='" & VSFG.TextMatrix(i, 39) & "'," & _
                         " per_codigo_ref='" & Trim(Replace(VSFG.TextMatrix(i, 40), "-", "")) & "'," & _
                         " per_codigo_ref2='" & Trim(Replace(VSFG.TextMatrix(i, 41), "-", "")) & "'," & _
                    ...
                    strSql = strSql & " per_observacion='" & UCase(VSFG.TextMatrix(i, 50)) & "'," & _
                         " per_fac_flete='" & Abs(FormatoD0(VSFG.TextMatrix(i, 52))) & "'," & _
                         " per_perdesde='" & VSFG.TextMatrix(i, 59) & "',"
                ElseIf TIPOUsu = "PV" Or TIPOUsu = "PR" Or TIPOUsu = "PRG" Or TIPOUsu = "PVG" Or TIPOUsu = "PVR" Then
                    ...
                    strSql = strSql & " zon_codigo='" & UCase(VSFG.TextMatrix(i, 18)) & "'," & _
                         ...
                         " for_ent_codigo='" & VSFG.TextMatrix(i, 27) & "',"
                    strSql = strSql & " per_observacion='" & UCase(VSFG.TextMatrix(i, 50)) & "'," & _
                         " per_fac_flete='" & Abs(FormatoD0(VSFG.TextMatrix(i, 52))) & "',"
                ElseIf TIPOUsu = "CA" Then
                
                    strSqlAux = " UPDATE persona " & _
                             " SET per_observacion_mod = CONCAT(COALESCE(per_observacion_mod,'')," & _
                            " IIF(per_bloqueado_g!='" & Abs(FormatoD0(VSFG.TextMatrix(i, 55))) & "'," & _
                                  " CONCAT(CURRENT_TIMESTAMP,' - " & strUsuario & "',' - CLIENTE ',IIF(per_bloqueado=0,'BLOQUEADO GERENCIA','DESBLOQUEADO GERENCIA'),'" & vbNewLine & "'), " & _
                                  " ''), " & _
                            " IIF(for_pag_codigo!='" & VSFG.TextMatrix(i, 30) & "'," & _
                                  " CONCAT(CURRENT_TIMESTAMP,' - " & strUsuario & "',' - FORMA DE PAGO ANTERIOR ',for_pag_codigo,'" & vbNewLine & "'), " & _
                                  " ''), " & _
                            " IIF(for_pag_codigo_imp!='" & VSFG.TextMatrix(i, 31) & "'," & _
                                  " CONCAT(CURRENT_TIMESTAMP,' - " & strUsuario & "',' - FORMA DE PAGO IMPRESION ANTERIOR ',for_pag_codigo_imp,'" & vbNewLine & "'), " & _
                                  " ''), "
                    strSqlAux = strSqlAux & " IIF(per_codigo_resp!='" & VSFG.TextMatrix(i, 37) & "'," & _
                                  " CONCAT(CURRENT_TIMESTAMP,' - " & strUsuario & "',' - DEUDOR ANTERIOR ',per_codigo_resp,'" & vbNewLine & "'), " & _
                                  " ''), " & _
                            " IIF(per_credito!='" & FormatoD2(VSFG.TextMatrix(i, 28)) & "'," & _
                                  " CONCAT(CURRENT_TIMESTAMP,' - " & strUsuario & "',' - CUPO DE CREDITO ANTERIOR ',per_credito,'" & vbNewLine & "'), " & _
                                  " ''), "
                    strSqlAux = strSqlAux & " IIF(per_pagare!='" & FormatoD2(VSFG.TextMatrix(i, 32)) & "'," & _
                                  " CONCAT(CURRENT_TIMESTAMP,' - " & strUsuario & "',' - PAGARE ANTERIOR ',per_pagare,'" & vbNewLine & "'), " & _
                                  " ''), " & _
                            " IIF(tip_gar_codigo!='" & VSFG.TextMatrix(i, 33) & "'," & _
                                  " CONCAT(CURRENT_TIMESTAMP,' - " & strUsuario & "',' - TIPO GARANTIA ANTERIOR ',tip_gar_codigo,'" & vbNewLine & "'), " & _
                                  " ''), " & _
                            " IIF(per_garantiasolidariareal!='" & UCase(VSFG.TextMatrix(i, 34)) & "'," & _
                                  " CONCAT(CURRENT_TIMESTAMP,' - " & strUsuario & "',' - GARANTIA SOLIDARIAREAL ANTERIOR ',per_garantiasolidariareal,'" & vbNewLine & "'), " & _
                                  " ''), " & _
                            " IIF(per_nombregarante!='" & UCase(VSFG.TextMatrix(i, 35)) & "'," & _
                                  " CONCAT(CURRENT_TIMESTAMP,' - " & strUsuario & "',' - GARANTE ANTERIOR ',per_nombregarante,'" & vbNewLine & "'), " & _
                                  " ''), " & _
                            " IIF(gar_aut_codigo!='" & VSFG.TextMatrix(i, 36) & "'," & _
                                  " CONCAT(CURRENT_TIMESTAMP,' - " & strUsuario & "',' - AUTORIZACION POR ANTERIOR ',gar_aut_codigo,'" & vbNewLine & "'), " & _
                                  " '')) "

                
                    strSql = " UPDATE persona " & _
                         " SET est_civ_codigo='" & UCase(Trim(VSFG.TextMatrix(i, 9))) & "'," & _
                         " ori_ing_codigo='" & UCase(Trim(VSFG.TextMatrix(i, 10))) & "'," & _
                         " ciu_codigo='" & UCase(VSFG.TextMatrix(i, 15)) & "'," & _
                         " dis_pol_codigo='" & UCase(VSFG.TextMatrix(i, 16)) & "',"
                    strSql = strSql & " per_direccion='" & UCase(VSFG.TextMatrix(i, 19)) & "'," & _
                         " per_ubicacion='" & UCase(VSFG.TextMatrix(i, 20)) & "'," & _
                         " per_telf='" & UCase(VSFG.TextMatrix(i, 21)) & "'," & _
                         " per_fax='" & UCase(VSFG.TextMatrix(i, 22)) & "'," & _
                         " per_celular='" & UCase(VSFG.TextMatrix(i, 23)) & "'," & _
                    ...
                    strSql = strSql & " per_observacion='" & UCase(VSFG.TextMatrix(i, 50)) & "'," & _
                         ...
                         " cob_codigo='" & VSFG.TextMatrix(i, 71) & "',"
                ElseIf TIPOUsu = "GE" Then
                    ...
                    strSql = strSql & " per_pagare='" & FormatoD2(VSFG.TextMatrix(i, 32)) & "'," & _
                         ...
                         " per_codigo_resp='" & Trim(Replace(VSFG.TextMatrix(i, 37), "-", "")) & "',"
                    strSql = strSql & " per_observacion='" & UCase(VSFG.TextMatrix(i, 50)) & "'," & _
                         " per_bloqueado_g='" & Abs(FormatoD0(VSFG.TextMatrix(i, 55))) & "',"
                Else
                
                    strSqlAux = " UPDATE persona " & _
                             " SET per_observacion_mod = CONCAT(COALESCE(per_observacion_mod,''),IIF(per_bloqueado!='" & Abs(FormatoD0(VSFG.TextMatrix(i, 54))) & "'," & _
                                  " CONCAT(CURRENT_TIMESTAMP,' - " & strUsuario & "',' - CLIENTE ',IIF(per_bloqueado=0,'BLOQUEADO CARTERA','DESBLOQUEADO CARTERA'),'" & vbNewLine & "'), " & _
                                  " ''), " & _
                            " IIF(per_bloqueado_g!='" & Abs(FormatoD0(VSFG.TextMatrix(i, 55))) & "'," & _
                                  " CONCAT(CURRENT_TIMESTAMP,' - " & strUsuario & "',' - CLIENTE ',IIF(per_bloqueado=0,'BLOQUEADO GERENCIA','DESBLOQUEADO GERENCIA'),'" & vbNewLine & "'), " & _
                                  " ''), " & _
                ...
                End If
                If strSqlAux <> "" Then
                    strSqlAux = strSqlAux & " WHERE per_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
                         " AND emp_codigo='" & strEmpresa & "' " & _
                         " AND cat_p_tipo='C'"
                    clsCon_Def.Ejecutar strSqlAux, "M"
                End If
                strSql = strSql & " per_fechamod=CURRENT_TIMESTAMP," & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            Else
                ...
                If FormatoD0(clsCon_Def.adorec_Def(0)) > 0 Then
                    ...
                    control = 1
                Else
                    If VSFG.TextMatrix(i, 39) = "PRO" Then
                    ...
                    ElseIf VSFG.TextMatrix(i, 39) = "GYE" Then
                        strSql = " SELECT CONCAT('C',FORMAT(ROUND(COALESCE(MAX(REPLACE(RIGHT(per_codigo,LEN(per_codigo)-1),'E','')+0),0)+1,0),'000000'),'E') as cod " & _
                                 " FROM persona " & _
                                 " WHERE cat_p_tipo='C'" & _
                                 " AND per_codigo like 'C%E'" & _
                                 " AND emp_codigo='" & strEmpresa & "'" & _
                                 " GROUP BY emp_codigo"
                    
                    ElseIf VSFG.TextMatrix(i, 39) = "REC" Then
                        strSql = " SELECT CONCAT('C',FORMAT(ROUND(COALESCE(MAX(REPLACE(RIGHT(per_codigo,LEN(per_codigo)-1),'F','')+0),0)+1,0),'000000'),'F') as cod " & _
                                 " FROM persona " & _
                                 " WHERE cat_p_tipo='C'" & _
                                 " AND per_codigo like 'C%F'" & _
                                 " AND emp_codigo='" & strEmpresa & "'" & _
                                 " GROUP BY emp_codigo"
                    
                    Else
                        strSql = " SELECT emp_codigo,CONCAT('C',FORMAT(ROUND(COALESCE(MAX(REPLACE(per_codigo,'C','0')+0),0)+1,0),'000000')) as cod  " & _
                                 " FROM persona " & _
                                 " WHERE cat_p_tipo='C'" & _
                                 " AND emp_codigo='" & strEmpresa & "'" & _
                                 " AND isnumeric(RIGHT(per_codigo,1))=1 " & _
                                 " GROUP BY emp_codigo"
                    End If
                    clsCon_Def.Ejecutar strSql
...
'                        End If
                        ...
                        'ENVIO DE CORREOS
                        If VSFG.TextMatrix(i, 39) = "JON" Then
                            strSql = " SELECT COALESCE(persona.per_email,'') as per_email,CONCAT(COALESCE(persona.per_apellido,''),' ',COALESCE(persona.per_nombre,'')) as cli, " & _
                                     " COALESCE(IIIF(LEN(CONCAT(COALESCE(N9.per_apellido,''),' ',COALESCE(N9.per_nombre,'')))>2,N9.per_email," & _
                            ...
                            If Trim(emailCliente) & Trim(emailPapacliente) & Trim(emailN1cliente) <> "" Then
                                ...
                                EnviarMail NombreComercial & " Servicio al Cliente", CorreoServicioAlCliente, ClienteNombre, emailCliente, CorreoAsistenteCartera, "Bienvenid@", _
...
Private Sub cmdReferido_Click()
    
    frmCargaReferido.txtCliente.Text = VSFG.TextMatrix(VSFG.Row, 6) & " " & VSFG.TextMatrix(VSFG.Row, 7)
    frmCargaReferido.txtCliente.Tag = VSFG.TextMatrix(VSFG.Row, 1)
    frmCargaReferido.Show
End Sub

Private Sub cmdVerReferido_Click()
    Dim Mensaje As String
    strSql = " SELECT CONCAT(per_apellido,' ',per_nombre) as cli" & _
             " FROM persona_referida INNER JOIN persona " & _
             " ON persona_referida.emp_codigo=persona.emp_codigo" & _
             " AND persona_referida.per_codigo=persona.per_codigo " & _
             " WHERE persona_referida.emp_codigo='" & strEmpresa & "' " & _
             " AND persona_referida.per_referido='" & VSFG.TextMatrix(VSFG.Row, 1) & "'" & _
             " ORDER BY CONCAT(per_apellido,' ',per_nombre)"
    clsCon_Def.Ejecutar strSql
    If clsCon_Def.adorec_Def.RecordCount > 0 Then
        Mensaje = "Referido por:" & vbNewLine
        While Not clsCon_Def.adorec_Def.EOF
            Mensaje = Mensaje & clsCon_Def.adorec_Def("cli") & vbNewLine
    ...
    strSql = " SELECT CONCAT(per_apellido,' ',per_nombre) as cli" & _
             " FROM persona_referida INNER JOIN persona " & _
             " ON persona_referida.emp_codigo=persona.emp_codigo" & _
             " AND persona_referida.per_referido=persona.per_codigo " & _
             " WHERE persona_referida.emp_codigo='" & strEmpresa & "' " & _
             " AND persona_referida.per_codigo='" & VSFG.TextMatrix(VSFG.Row, 1) & "'" & _
             " ORDER BY CONCAT(per_apellido,' ',per_nombre)"
    clsCon_Def.Ejecutar strSql
    If clsCon_Def.adorec_Def.RecordCount > 0 Then
        Mensaje = Mensaje & vbNewLine & "Refiere a:" & vbNewLine
        While Not clsCon_Def.adorec_Def.EOF
            Mensaje = Mensaje & clsCon_Def.adorec_Def("cli") & vbNewLine
            clsCon_Def.adorec_Def.MoveNext
        Wend
    ...
    If Trim(Mensaje) <> "" Then
        MsgBox Mensaje, , "Referidos"
    Else
        MsgBox "No tiene Referente ni Referidos", , "Referidos"
    End If
End Sub
...
Private Sub VSFG_BeforeRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long, Cancel As Boolean)
    ...
    If OldCol = 14 Then
        If Abs(Val(VSFG.TextMatrix(OldRow, VSFG.Cols - 1))) = 2 Or Abs(Val(VSFG.TextMatrix(OldRow, VSFG.Cols - 1))) = 3 Then
            ...
            Cedula10 = Left(Cedula, 10)
            If VerificaCedula(Cedula10) = False Then
                Cancel = True
            Else
                If Left(Cedula10, 1) = "P" Then
                    Cedula = "P" & Cedula
                End If
...
Private Sub VSFG_CellButtonClick(ByVal Row As Long, ByVal Col As Long)
    If Col = 51 Then
        ...
        frmContacto.Show 1
    ElseIf Col = 4 Then
        frmRed.Negocio = cmbNegocio.BoundText
...
Private Sub VSFG_BeforeEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    ElseIf Val(VSFG.TextMatrix(Row, VSFG.Cols - 1)) = 2 Or Val(VSFG.TextMatrix(Row, VSFG.Cols - 1)) = -2 Then
        If TIPOUsu = "AC" Or TIPOUsu = "SAC" Then
        ...
        ElseIf TIPOUsu = "JSA" Then
            ...
            VSFG.TextMatrix(Row, 72) = 0
            VSFG.TextMatrix(Row, 73) = 0
            If Col = 28 Or (30 <= Col And Col <= 37) Or Col = 54 Or Col = 55 Or Col >= VSFG.Cols - 6 Then
                Cancel = True
            End If
        ElseIf TIPOUsu = "PV" Or TIPOUsu = "PR" Or TIPOUsu = "PRG" Or TIPOUsu = "PVG" Or TIPOUsu = "PVR" Then
            VSFG.TextMatrix(Row, 28) = 0
            ...
            ElseIf TIPOUsu = "PVR" Then
                VSFG.TextMatrix(Row, 11) = "REC"
                VSFG.TextMatrix(Row, 39) = "REC"
            ElseIf TIPOUsu = "PRG" Then
                VSFG.TextMatrix(Row, 11) = "PRO"
                VSFG.TextMatrix(Row, 39) = "GYE"
            Else
                VSFG.TextMatrix(Row, 11) = "PRO"
    ...
    ElseIf Val(VSFG.TextMatrix(Row, VSFG.Cols - 1)) = 3 Or Val(VSFG.TextMatrix(Row, VSFG.Cols - 1)) = -3 Then
        If TIPOUsu = "JSA" Then
            ...
            End If
        ElseIf TIPOUsu = "SAC" Then
            If Col = 1 Or Col = 11 Or Col = 12 Or Col = 13 Or (Col >= 28 And Col <= 39) Or (Col >= 53 And Col <= 58) Or (Col >= 60) Or Col >= VSFG.Cols - 6 Then
                Cancel = True
            End If
        ElseIf TIPOUsu = "PV" Or TIPOUsu = "PR" Or TIPOUsu = "PRG" Or TIPOUsu = "PVG" Or TIPOUsu = "PVR" Then
        ...
        ElseIf TIPOUsu = "GE" Then
            ...
            End If
        Else
            If Col = 1 Or Col >= VSFG.Cols - 6 Then
                Cancel = True
            End If
        End If
    End If
...
Private Sub Form_Load()
    ...
    ElseIf TIPOUsu = "SU" Or TIPOUsu = "SAC" Or TIPOUsu = "JSA" Or TIPOUsu = "PV" _
        Or TIPOUsu = "PVG" Or TIPOUsu = "PVR" Or TIPOUsu = "PR" Or TIPOUsu = "PRG" Then
        ucrtVSFG.Inicializar , False, , , , , , , , "909"
    End If
    IniDato
    ...
    If cmbNegocio.BoundText = "" Then
        ...
        If clsAComFiltro.adorec_Def.RecordCount > 0 Then
            cmbNegocio.BoundText = clsAComFiltro.adorec_Def(0)
        End If
    End If
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    ElseIf KeyCode = vbKeyF6 And Shift = 1 Then
        ...
        strUsuWeb = InputBox("Ingrese el usuario", "Cambio de contraseña")
        strPasWeb = InputBox("Ingrese la Clave", "Cambio de contraseña")
        strSql = " UPDATE usuario_web " & _
                 " SET usu_web_clave=SUBSTRING(sys.fn_sqlvarbasetostr(HASHBYTES('MD5','" & strPasWeb & "')),3,32)," & _
                 " usu_web_fechamod=CURRENT_TIMESTAMP, " & _
                 " usu_web_usuwebmod='" & strUsuario & "'" & _
                 " WHERE usu_web_usuario='" & strUsuWeb & "'"
...
Private Sub VSFG_KeyPress(KeyAscii As Integer)
    If TIPOUsu = "SU" Or TIPOUsu = "PR" Or TIPOUsu = "PRG" Or TIPOUsu = "CA" Then
