...
Begin VB.Form frmATED 
   ...
   Begin TabDlg.SSTab sstI 
      ...
      Tab(1).Control(0)=   "chkAutoBusquedaCompras"
      Tab(1).Control(1)=   "cmdExportarCompras"
      Tab(1).Control(2)=   "cmdImportarCompras"
      Tab(1).Control(3)=   "vsfgDescCompras"
      Tab(1).Control(4)=   "vsfgCompras"
      Tab(1).ControlCount=   5
      TabCaption(2)   =   "3. Transacciones Locales - VENTAS"
      TabPicture(2)   =   "frmATED.frx":157E
      Tab(2).ControlEnabled=   0   'False
      Tab(2).Control(0)=   "chkAutoBusquedaVentas"
      Tab(2).Control(1)=   "cmdJuntar"
      Tab(2).Control(2)=   "cmdExportarVentas"
      Tab(2).Control(3)=   "cmdImportarVentas"
      Tab(2).Control(4)=   "vsfgDescVentas"
      Tab(2).Control(5)=   "vsfgVentas"
      ...
      Tab(9).Control(0)=   "WebBrow"
      Tab(9).Control(1)=   "cmdGenerarXML"
...
'    fechaFIN = DateAdd("M", 1, fechaINI) - 1
    ...
    ElseIf sstI.Tab = 10 Then
        ...
        strSql = " DROP TABLE IF EXISTS compra "
        clsCon_Def.Ejecutar strSql
        strSql = " CREATE TEMPORARY TABLE compra (emp_codigo char(3),cue_p_c_codigo decimal(11,0)," & _
                 " cue_p_c_tipo char(2),ivaprod decimal(14,2),ivaserv decimal(14,2)," & _
                 " ret_codigo1 char(3), ret_codigo2 char(3)," & _
                 " key pri (emp_codigo ,cue_p_c_codigo ,cue_p_c_tipo)) "
        clsCon_Def.Ejecutar strSql
        strSql = " INSERT into compra " & _
                 " SELECT cuenta_p_c.emp_codigo,cuenta_p_c.cue_p_c_codigo,cuenta_p_c.cue_p_c_tipo, " & _
                 " COALESCE(IF(ROUND(cuenta_p_c.cue_p_c_st_prod*cod_iva_porcentaje/100,2)=RIVP.det_com_ret_valor," & _
                 " ROUND(RIVP.det_com_ret_valor*RIVP.det_com_ret_porcentaje/100,2),0),0) AS valorRetBienes," & _
                 " COALESCE(IF(ROUND(cuenta_p_c.cue_p_c_st_serv*cod_iva_porcentaje/100,2)=RIVP.det_com_ret_valor," & _
                 " ROUND(RIVP.det_com_ret_valor*RIVP.det_com_ret_porcentaje/100,2),0),0) AS valorRetServicios," & _
                 " COALESCE(IF(ROUND(cuenta_p_c.cue_p_c_st_prod*cod_iva_porcentaje/100,2)=RIVP.det_com_ret_valor," & _
                 " RIVP.ret_codigo,0),0) AS porRetBienes," & _
                 " COALESCE(IF(ROUND(cuenta_p_c.cue_p_c_st_serv*cod_iva_porcentaje/100,2)=RIVP.det_com_ret_valor,RIVP.ret_codigo,0),0) AS porRetServicios " & _
                 ...
                 " INNER JOIN det_comp_ret as RIVP ON cuenta_p_c.emp_codigo=RIVP.emp_codigo AND cuenta_p_c.cue_p_c_codigo=RIVP.cue_p_c_codigo AND cuenta_p_c.cue_p_c_tipo=RIVP.cue_p_c_tipo  AND RIVP.ret_codigo<100 " & _
                 " INNER JOIN retencion AS retencion2 ON retencion2.ret_codigo=RIVP.ret_codigo AND retencion2.emp_codigo=RIVP.emp_codigo " & _
                 "  " & _
                 " INNER JOIN retencion ON RIVP.emp_codigo=retencion.emp_codigo AND RIVP.ret_codigo=retencion.ret_codigo" & _
                 "  " & _
                 " INNER JOIN codigo_iva on cuenta_p_c.cod_iva_codigo=codigo_iva.cod_iva_codigo and codigo_iva.cod_iva_codigo='2' " & _
                 " WHERE cuenta_p_c.emp_codigo='" & strEmpresa & "' AND cuenta_p_c.cue_p_c_fechaemision BETWEEN '" & fechaINI & "' AND '" & fechaFIN & "' " & _
                 " AND cuenta_p_c.cue_p_c_tipo='P'  AND tip_doc_cue_codigo>0  AND cue_p_c_valor!=0"
        clsCon_Def.Ejecutar strSql
        strSql = " DROP TABLE IF EXISTS ret "
        clsCon_Def.Ejecutar strSql
        strSql = " CREATE TEMPORARY TABLE ret (emp_codigo char(3),cue_p_c_codigo decimal(11,0),cue_p_c_tipo char(2),codRetAir char(3)," & _
                 " baseImpAir decimal(14,2), porcentajeAir decimal(5,2),valRetAir decimal(14,2), " & _
                 " key pri (emp_codigo ,cue_p_c_codigo ,cue_p_c_tipo)) "
        clsCon_Def.Ejecutar strSql
        strSql = " INSERT INTO ret " & _
                 " SELECT RIR.emp_codigo,RIR.cue_p_c_codigo,RIR.cue_p_c_tipo,  COALESCE(RIR.ret_codigo,'') AS codRetAir," & _
                 " COALESCE(RIR.det_com_ret_valor/IF(RIR.ret_codigo='322',10,1),'') AS baseImpAir," & _
                 " COALESCE(RIR.det_com_ret_porcentaje*IF(RIR.ret_codigo='322',10,1),'') AS porcentajeAir," & _
                 " COALESCE(RIR.det_com_ret_valor*RIR.det_com_ret_porcentaje/100,'') AS valRetAir" & _
                 " FROM cuenta_p_c INNER JOIN comprobante_retencion ON cuenta_p_c.emp_codigo=comprobante_retencion.emp_codigo AND cuenta_p_c.cue_p_c_codigo=comprobante_retencion.cue_p_c_codigo AND cuenta_p_c.cue_p_c_tipo=comprobante_retencion.cue_p_c_tipo " & _
                 " INNER JOIN det_comp_ret as RIR ON cuenta_p_c.emp_codigo=RIR.emp_codigo AND cuenta_p_c.cue_p_c_tipo=RIR.cue_p_c_tipo AND cuenta_p_c.cue_p_c_codigo=RIR.cue_p_c_codigo " & _
                 " INNER JOIN retencion ON retencion.ret_codigo=RIR.ret_codigo AND retencion.emp_codigo=RIR.emp_codigo AND RIR.ret_codigo>100 " & _
                 "  " & _
                 " WHERE cuenta_p_c.emp_codigo='" & strEmpresa & "' AND cuenta_p_c.cue_p_c_fechaemision BETWEEN '" & fechaINI & "' AND '" & fechaFIN & "' " & _
                 " AND cuenta_p_c.cue_p_c_tipo='P'  AND tip_doc_cue_codigo>0  AND cue_p_c_valor!=0"
        clsCon_Def.Ejecutar strSql
        '" if(cue_p_c_st_prod+cue_p_c_st_serv=0 and cue_p_c_st_cero=0,cuenta_p_c.cue_p_c_valor/1.12,cue_p_c_st_prod+cue_p_c_st_serv) as baseImpgrav, " &
        strSql = " SELECT DATE_FORMAT(cue_p_c_fechaemision,'%m/%Y') as anomes, '01' as TipoItem1, " & _
                 " IF(LENGTH(per_ruc)=13,'01','02') as tpIdProv, per_ruc as idProv," & _
                 ...
                 " DATE_FORMAT(cue_p_c_fechaemision,'%d/%m/%Y') as fechaEmision, COALESCE(com_ret_autorizacion,'') as autRetencion," & _
                 " COALESCE(LEFT(com_ret_serie,3),'') as estabRetencion, COALESCE(RIGHT(com_ret_serie,3),'') as ptoEmiRetencion," & _
                 " COALESCE(com_ret_numero,'') as secRetencion, COALESCE(DATE_FORMAT(com_ret_fecha,'%d/%m/%Y'),'') as fechaEmiRet," & _
                 ...
                 " COALESCE(DATE_FORMAT(com_ret_fecha,'%d/%m/%Y'),'') as fechaEmiRet1," & _
                 ...
                 " LEFT JOIN ret ON comprobante_retencion.emp_codigo=ret.emp_codigo AND comprobante_retencion.cue_p_c_codigo=ret.cue_p_c_codigo AND comprobante_retencion.cue_p_c_tipo=ret.cue_p_c_tipo" & _
                 " LEFT JOIN compra ON compra.emp_codigo=comprobante_retencion.emp_codigo AND compra.cue_p_c_codigo=comprobante_retencion.cue_p_c_codigo AND compra.cue_p_c_tipo= comprobante_retencion.cue_p_c_tipo" & _
                 " WHERE cue_p_c_fechaemision BETWEEN '" & fechaINI & "' AND '" & fechaFIN & "'" & _
                 ...
                 " ORDER BY DATE_FORMAT(cue_p_c_fechaemision,'%Y/%m'),per_ruc,cue_p_c_serie,cue_p_c_numero"
        clsCon_Def.Ejecutar strSql
    ...
    ElseIf sstI.Tab = 2 Then
        ...
        vsfgVentas.Cell(flexcpBackColor, 1, 0, vsfgVentas.Rows - 1, vsfgVentas.Cols - 1) = vbWhite
            ...
            strSql = " DROP TABLE IF EXISTS venta "
            clsCon_Def.Ejecutar strSql
            
            strSql = " CREATE TEMPORARY TABLE venta (emp_codigo char(3),cue_p_c_codigo decimal(11,0),cue_p_c_tipo char(2),ivaret decimal(14,2), key pri(emp_codigo ,cue_p_c_codigo ,cue_p_c_tipo)) "
            clsCon_Def.Ejecutar strSql
            
            strSql = " INSERT into venta " & _
                     " SELECT cuenta_p_c.emp_codigo,cuenta_p_c.cue_p_c_codigo,cuenta_p_c.cue_p_c_tipo, " & _
                     " SUM(COALESCE(ROUND(RIVP.det_com_ret_valor*RIVP.det_com_ret_porcentaje/100,2),0)) AS ivaret " & _
                     " From cuenta_p_c " & _
            ...
            strSql = " CREATE TEMPORARY TABLE ret (emp_codigo char(3),cue_p_c_codigo decimal(11,0),cue_p_c_tipo char(2),baseImpAir decimal(14,2),valRetAir decimal(16,5)) "
            clsCon_Def.Ejecutar strSql
            
            strSql = " insert into ret " & _
                     " SELECT RIR.emp_codigo,RIR.cue_p_c_codigo,RIR.cue_p_c_tipo, " & _
                     " SUM(COALESCE(RIR.det_com_ret_valor,'')) AS baseImpAir, " & _
                     " SUM(COALESCE(ROUND(RIR.det_com_ret_valor*RIR.det_com_ret_porcentaje/100,5),'')) AS valRetAir " & _
                     " From cuenta_p_c " & _
                     " INNER JOIN pago ON cuenta_p_c.emp_codigo=pago.emp_codigo " & _
                     " AND cuenta_p_c.cue_p_c_codigo=pago.cue_p_c_codigo  AND cuenta_p_c.cue_p_c_tipo=pago.cue_p_c_tipo " & _
                     " AND pago.pag_observacion NOT LIKE '%ANULAD%' " & _
           ...
           strSql = " SELECT cue_p_c_egr_codigo as Item," & _
                    " IF(LEFT(per_ruc,1)='P','06',IF(LENGTH(per_ruc)=13,'04',IF(LENGTH(per_ruc)<=8,'07','05'))) as tpIdCliente," & _
                    " IF(LEFT(per_ruc,1)='P',MID(per_ruc,2,13),IF(LENGTH(per_ruc)<=8,'9999999999999',per_ruc)) as idCliente,'18' as tipoComprobante, " & _
                    " 1 as numeroComprobantes, " & _
                    " 0 as baseNoGraIva, ROUND(cue_p_c_st_cero+IF(cue_p_c_st_cero=0,COALESCE(det_egr_cantidad,0)*COALESCE(det_egr_precio,0),0),2) as baseImponible, " & _
                    " ROUND(egr_subtotal-egr_dcto-IF(cue_p_c_st_cero=0,COALESCE(det_egr_cantidad,0)*COALESCE(det_egr_precio,0),0),2) as baseImpGrav," & _
                    " ROUND(cue_p_c_iva,2) as montoIva, " & _
                    " 0 as valorRetIva, " & _
                    " 0 as valRetAir " & _
            ...
            strSql = strSql & " SELECT cue_p_c_egr_codigo as Item," & _
                    " IF(LEFT(per_ruc,1)='P','06',IF(LENGTH(per_ruc)=13,'04',IF(LENGTH(per_ruc)<=8,'07','05'))) as tpIdCliente," & _
                    " IF(LEFT(per_ruc,1)='P',MID(per_ruc,2,13),IF(LENGTH(per_ruc)<=8,'9999999999999',per_ruc)) as idCliente,'18' as tipoComprobante, " & _
                    " 0 as numeroComprobantes, " & _
                    " 0 as baseNoGraIva, 0 as baseImponible, " & _
                    " 0 as baseImpGrav," & _
                    " 0 as montoIva, " & _
                    " COALESCE(ivaret,0) as valorRetIva, " & _
            ...
            strSql = strSql & " INNER JOIN comprobante_retencion ON cuenta_p_c.emp_codigo=comprobante_retencion.emp_codigo " & _
                    ...
                    " INNER JOIN ret ON comprobante_retencion.emp_codigo=ret.emp_codigo " & _
                    " AND comprobante_retencion.cue_p_c_codigo=ret.cue_p_c_codigo " & _
                    " AND comprobante_retencion.cue_p_c_tipo=ret.cue_p_c_tipo " & _
                    " LEFT JOIN venta  ON comprobante_retencion.emp_codigo=venta.emp_codigo " & _
            ...
            strSql = strSql & " WHERE cuenta_p_c.cue_p_c_tipo='C'  AND tip_doc_cue_codigo>0  AND cue_p_c_valor!=0 " & _
                    " AND cuenta_p_c.emp_codigo='" & strEmpresa & "'  "
            strSql = strSql & " UNION "
            strSql = strSql & " SELECT STRAIGHT_JOIN ing_codigo as Item,IF(LEFT(per_ruc,1)='P','06',IF(LENGTH(per_ruc)=13,'04',IF(LENGTH(per_ruc)<=8,'07','05'))) as tpIdCliente,IF(LEFT(per_ruc,1)='P',MID(per_ruc,2,13),IF(LENGTH(per_ruc)<=8,'9999999999999',per_ruc)) as idCliente,'04' as tipoComprobante, " & _
                    " 1 as numeroComprobantes, " & _
                    " 0 as baseNoGraIva,  (ing_subtotal_o-IF(ing_subtotal=0 AND ing_impuesto=0,ing_dcto,0)) as baseImponible,ing_subtotal-IF(ing_subtotal=0 AND ing_impuesto=0,0,ing_dcto) as baseImpGrav, " & _
                    " ing_impuesto as montoIva,0 as valorRetIva,0 as valRetAir " & _
...
'            vsfgVentas.TextMatrix(i, 43) = clsCon_Def.adorec_Def(43)
    ...
    ElseIf sstI.Tab = 11 Then
        ...
        vsfgVentasEstablecimiento.Cell(flexcpBackColor, 1, 0, vsfgVentasEstablecimiento.Rows - 1, vsfgVentasEstablecimiento.Cols - 1) = vbWhite
            ...
            strSql = " CREATE TEMPORARY TABLE venEsta (codEstab char(3), ventasEstab decimal(14,2)) "
            clsCon_Def.Ejecutar strSql
            
           strSql = " INSERT INTO venEsta " & _
                    " SELECT RIGHT(cuenta_p_c.cue_p_c_serie,3) as codEstab, " & _
                    " SUM(ROUND(cue_p_c_st_cero+IF(cue_p_c_st_cero=0,COALESCE(det_egr_cantidad,0)*COALESCE(det_egr_precio,0),0),2) + " & _
                    " ROUND(egr_subtotal-egr_dcto-IF(cue_p_c_st_cero=0,COALESCE(det_egr_cantidad,0)*COALESCE(det_egr_precio,0),0),2)) as ventasEstab" & _
                    " From cuenta_p_c INNER JOIN egreso ON cuenta_p_c.emp_codigo=egreso.emp_codigo " & _
                    " AND cuenta_p_c.cue_p_c_egr_codigo=egreso.egr_codigo " & _
                    " AND cuenta_p_c.per_codigo=egreso.per_codigo  " & _
                    " AND egreso.tip_egr_codigo='FAC' AND egreso.egr_anulado=0 " & _
            ...
            strSql = strSql & " WHERE cue_p_c_fechaemision BETWEEN '" & fechaINI & "' AND '" & fechaFIN & "' " & _
                    " AND cuenta_p_c.cue_p_c_tipo='C'  AND tip_doc_cue_codigo>0  AND cue_p_c_valor!=0 " & _
                    " AND cuenta_p_c.emp_codigo='" & strEmpresa & "' GROUP BY codEstab  "
            clsCon_Def.Ejecutar strSql
            strSql = " INSERT INTO venEsta " & _
                    " SELECT STRAIGHT_JOIN LEFT(ing_serie,3) as codEstab," & _
                    " -1*SUM((ing_subtotal_o-IF(ing_subtotal=0 AND ing_impuesto=0,ing_dcto,0)) +" & _
                    " (ing_subtotal-IF(ing_subtotal=0 AND ing_impuesto=0,0,ing_dcto))) as ventasEstab " & _
                    " FROM ingreso INNER JOIN persona ON ingreso.emp_codigo=persona.emp_codigo " & _
                    " AND ingreso.per_codigo=persona.per_codigo " & _
                    " WHERE ing_fecha Between '" & fechaINI & "' AND '" & fechaFIN & "'  " & _
                    " AND tip_ing_codigo='DCL'  AND ing_anulado=0 " & _
        ...
        Else
            ...
            strSql = " CREATE TEMPORARY TABLE compra (emp_codigo char(3),cue_p_c_codigo decimal(11,0),cue_p_c_tipo char(2),iva30 decimal(14,2),iva10 decimal(14,2),iva20 decimal(14,2),iva70 decimal(14,2),iva100 decimal(14,2), key pri (emp_codigo ,cue_p_c_codigo ,cue_p_c_tipo)) "
            clsCon_Def.Ejecutar strSql
            
            strSql = " INSERT into compra " & _
                    " SELECT cuenta_p_c.emp_codigo,cuenta_p_c.cue_p_c_codigo,cuenta_p_c.cue_p_c_tipo, " & _
                    " SUM(COALESCE(IF(RIVP.det_com_ret_porcentaje=10,ROUND(RIVP.det_com_ret_valor*RIVP.det_com_ret_porcentaje/100,2),0),0)) AS iva10, " & _
                    " SUM(COALESCE(IF(RIVP.det_com_ret_porcentaje=20,ROUND(RIVP.det_com_ret_valor*RIVP.det_com_ret_porcentaje/100,2),0),0)) AS iva20, " & _
                    " SUM(COALESCE(IF(RIVP.det_com_ret_porcentaje=30,ROUND(RIVP.det_com_ret_valor*RIVP.det_com_ret_porcentaje/100,2),0),0)) AS iva30, " & _
                    " SUM(COALESCE(IF(RIVP.det_com_ret_porcentaje=70,ROUND(RIVP.det_com_ret_valor*RIVP.det_com_ret_porcentaje/100,2),0),0)) AS iva70, " & _
                    " SUM(COALESCE(IF(RIVP.det_com_ret_porcentaje=100,ROUND(RIVP.det_com_ret_valor*RIVP.det_com_ret_porcentaje/100,2),0),0)) AS iva100 " & _
                    " From cuenta_p_c " & _
                    " INNER JOIN comprobante_retencion ON cuenta_p_c.emp_codigo=comprobante_retencion.emp_codigo " & _
                    " AND cuenta_p_c.cue_p_c_codigo=comprobante_retencion.cue_p_c_codigo  AND cuenta_p_c.cue_p_c_tipo=comprobante_retencion.cue_p_c_tipo " & _
                    " INNER JOIN det_comp_ret as RIVP ON cuenta_p_c.emp_codigo=RIVP.emp_codigo " & _
                    ...
                    " INNER JOIN codigo_iva on cuenta_p_c.cod_iva_codigo=codigo_iva.cod_iva_codigo and codigo_iva.cod_iva_codigo='2' " & _
                    " WHERE cuenta_p_c.emp_codigo='" & strEmpresa & "' " & _
                    " AND cue_p_c_fechaemision BETWEEN '" & fechaINI & "' AND '" & fechaFIN & "' " & _
                    " AND cuenta_p_c.cue_p_c_tipo='P' AND cue_p_c_valor!=0 " & _
                    " GROUP BY cuenta_p_c.emp_codigo,cuenta_p_c.cue_p_c_codigo,cuenta_p_c.cue_p_c_tipo"
            clsCon_Def.Ejecutar strSql
            ...
            strSql = "CREATE TEMPORARY TABLE ret (emp_codigo char(3),cue_p_c_codigo decimal(11,0),cue_p_c_tipo char(2),codRetAir char(3),baseImpAir decimal(14,2), porcentajeAir decimal(5,2),valRetAir decimal(16,5)) "
            clsCon_Def.Ejecutar strSql
            
            strSql = "INSERT INTO ret " & _
                    " SELECT RIR.emp_codigo,RIR.cue_p_c_codigo,RIR.cue_p_c_tipo, " & _
                    " COALESCE(RIR.ret_codigo,'') AS codRetAir, " & _
                    " COALESCE(RIR.det_com_ret_valor/IF(RIR.ret_codigo='322',10,1),'') AS baseImpAir, " & _
                    " COALESCE(RIR.det_com_ret_porcentaje*IF(RIR.ret_codigo='322',10,1),'') AS porcentajeAir, " & _
                    " COALESCE(ROUND(RIR.det_com_ret_valor*RIR.det_com_ret_porcentaje/100,5),'') AS valRetAir " & _
                    " From cuenta_p_c " & _
                    " INNER JOIN comprobante_retencion ON cuenta_p_c.emp_codigo=comprobante_retencion.emp_codigo " & _
                    " AND cuenta_p_c.cue_p_c_codigo=comprobante_retencion.cue_p_c_codigo  AND cuenta_p_c.cue_p_c_tipo=comprobante_retencion.cue_p_c_tipo " & _
                    " INNER JOIN det_comp_ret as RIR " & _
                    ...
                    " AND cuenta_p_c.cue_p_c_codigo =RIR.cue_p_c_codigo " & _
                    " INNER JOIN retencion ON retencion.ret_codigo=RIR.ret_codigo AND retencion.emp_codigo=RIR.emp_codigo " & _
                    " AND RIR.ret_codigo>100 " & _
                    " WHERE cuenta_p_c.emp_codigo='" & strEmpresa & "' " & _
                    " AND cue_p_c_fechaemision BETWEEN '" & fechaINI & "' AND '" & fechaFIN & "' " & _
                    " AND cuenta_p_c.cue_p_c_tipo='P' AND cue_p_c_valor!=0 "
            clsCon_Def.Ejecutar strSql
            
            strSql = " SELECT '        ' as Item,IF(cod_sus_com_codigo='' OR cod_sus_com_codigo is null,'01',cod_sus_com_codigo) as codSustento," & _
                     " IF(LEFT(per_ruc,1)='P','03',IF(LENGTH(per_ruc)=13,'01','02')) as tpIdProv, " & _
                     " IF(LEFT(per_ruc,1)='P',MID(per_ruc,2,13),per_ruc) as idProv,tip_doc_cue_codigo as tipoComprobante," & _
                     " IF(LEFT(per_ruc,1)='P','02','') as tipoProv, IF(LEFT(per_ruc,1)='P','NO','') as parteRel, " & _
                     " DATE_FORMAT(cue_p_c_fechaemision,'%d/%m/%Y') as fechaRegistro, " & _
                     " LEFT(cue_p_c_serie,3) as establecimiento,RIGHT(cue_p_c_serie,3) as puntoEmision,cue_p_c_numero as secuencial, " & _
                     " DATE_FORMAT(cue_p_c_fechaemision,'%d/%m/%Y') as fechaEmision,cue_p_c_autorizacion as autorizacion, " & _
                     " 0 as baseNoGraIva, cue_p_c_st_cero as baseImponible, " & _
                     " cue_p_c_st_prod+cue_p_c_st_serv as baseImpgrav,'0' as baseImpExe,cue_p_c_ice as montoIce,Coalesce(cue_p_c_iva,'') as montoIva, " & _
            ...
            strSql = strSql & " '01' as pagoLocExt, 'NA' as paisEfecPago, 'NA' as aplicConvDobTrib, 'NA' as pagExtSujRetNorLeg,'NO' as pagoRegFis," & _
                     " IF(cue_p_c_st_cero+cue_p_c_st_prod+cue_p_c_st_serv+cue_p_c_ice+cue_p_c_iva>1000,'02','') as formaPag,'' as docModificado, '' as estabModificado,'' as ptoEmiModificado,'' as secModificado," & _
                     ...
                     " '' as autorizacionReemb,'' as baseImponibleReemb, '' as baseImpGravReemb,'' as baseNoGraIvaReemb,'' as baseImpExeReemb,'' as totbasesImpReemb," & _
                     ...
                     " COALESCE(com_ret_numero,'') as secRetencion1,  COALESCE(doc_ele_autorizacion,com_ret_autorizacion,'') as autRetencion1, " & _
                     " COALESCE(DATE_FORMAT(com_ret_fecha,'%d/%m/%Y'),'') as fechaEmiRet1, " & _
                     " COALESCE(codRetAir,'332') as codRetAir, " & _
                     " IF(baseImpAir<>0 or baseImpAir IS NOT NULL,baseImpAir ,if(cue_p_c_st_prod+cue_p_c_st_serv=0 and cue_p_c_st_cero=0,cuenta_p_c.cue_p_c_valor/1." & PorIVA & ",cue_p_c_st_prod+cue_p_c_st_serv+cue_p_c_st_cero)) as baseImpAir, " & _
            ...
            strSql = strSql & " From cuenta_p_c " & _
                     ...
                     " AND cuenta_p_c.cue_p_c_codigo=comprobante_retencion.cue_p_c_codigo " & _
                     " AND cuenta_p_c.cue_p_c_tipo=comprobante_retencion.cue_p_c_tipo " & _
                     " LEFT JOIN ret  ON comprobante_retencion.emp_codigo=ret.emp_codigo " & _
                     " AND comprobante_retencion.cue_p_c_codigo=ret.cue_p_c_codigo " & _
                     " AND comprobante_retencion.cue_p_c_tipo=ret.cue_p_c_tipo " & _
                     " LEFT JOIN compra " & _
                     " on compra.emp_codigo=comprobante_retencion.emp_codigo " & _
                     " and compra.cue_p_c_codigo=comprobante_retencion.cue_p_c_codigo " & _
                     " AND compra.cue_p_c_tipo= comprobante_retencion.cue_p_c_tipo "
            ...
            strSql = strSql & " Union " & _
                    " SELECT egr_numero as Item,'06' as codSustento," & _
                    " IF(LEFT(per_ruc,1)='P','03',IF(LENGTH(per_ruc)=13,'01','02')) as tpIdProv," & _
                    " IF(LEFT(per_ruc,1)='P',MID(per_ruc,2,13),per_ruc) as idProv,'04' as tipoComprobante, IF(LEFT(per_ruc,1)='P','02','') as tipoProv, IF(LEFT(per_ruc,1)='P','NO','') as parteRel, " & _
                    " DATE_FORMAT(egr_fecha,'%d/%m/%Y') as fechaRegistro, " & _
                    " LEFT(egr_serie,3) as establecimiento,RIGHT(egr_serie,3) as puntoEmision,egr_numero as secuencial," & _
                    " DATE_FORMAT(egr_fecha,'%d/%m/%Y') as fechaEmision,egr_autorizacion as autorizacion, " & _
                    " 0 as baseNoGraIva, ROUND((egr_subtotal_o-IF(egr_subtotal=0 AND egr_impuesto=0,egr_dcto,0)),2) as baseImponible, " & _
                     " ROUND((egr_subtotal-IF(egr_subtotal=0 AND egr_impuesto=0,0,egr_dcto)),2) as baseImpgrav,'0' as baseImpExe,0 as montoIce,ROUND(egr_impuesto,2) as montoIva, " & _
                     " 0 as valRetBien10, " & _
                     " 0 as valRetServ20, " & _
            ...
            If FormatoD0(vsfgCompras.TextMatrix(i, 4)) = 4 Then
                strSql = " SELECT DISTINCT cuenta_p_c.* FROM cuenta_p_c INNER JOIN pago " & _
                ...
                If cls_Aux.adorec_Def.RecordCount > 0 Then
                    vsfgCompras.TextMatrix(i, 30) = 1
                    vsfgCompras.TextMatrix(i, 31) = Left(cls_Aux.adorec_Def("cue_p_c_serie"), 3)
                    vsfgCompras.TextMatrix(i, 32) = Right(cls_Aux.adorec_Def("cue_p_c_serie"), 3)
                    vsfgCompras.TextMatrix(i, 33) = cls_Aux.adorec_Def("cue_p_c_numero")
                    vsfgCompras.TextMatrix(i, 34) = cls_Aux.adorec_Def("cue_p_c_autorizacion")
                End If
            End If
'            vsfgCompras.ShowCell i, 33
'            vsfgCompras.TextMatrix(i, 33) = CStr(clsCon_Def.adorec_Def(33))
...
Private Sub cmdJuntar_Click()
    ...
    With vsfgVentas
        ...
        While salir = False
            ...
            ii = rr2
            If Right(.TextMatrix(i, 2), 10) = "0100766963" Then
                MsgBox "0100766963"
            End If
            ...
            If Format(.TextMatrix(i, 1), "00") = Format(.TextMatrix(ii, 1), "00") And _
               ...
               1 = 1 And _
               1 = 1 Then
                .Cell(flexcpText, r1, 4, r2, 4) = Val(.TextMatrix(i, 4)) + Val(.TextMatrix(ii, 4))
                .Cell(flexcpText, r1, 5, r2, 5) = Val(.TextMatrix(i, 5)) + Val(.TextMatrix(ii, 5))
                .Cell(flexcpText, r1, 6, r2, 6) = Val(.TextMatrix(i, 6)) + Val(.TextMatrix(ii, 6))
...
Dim Valor As Variant
'    If VSFG.Cell(flexcpTextDisplay, Row, vsfgDesc.TextMatrix(Col, 13)) = "1791901215001" Then
'        MsgBox "AA"
'    End If
            ...
            If LineaAmarilla(vsfgDescCompras, vsfgCompras, i) = False Then
                ...
                For j = 1 To vsfgDescCompras.Rows - 1
                    ...
                    If vsfgDescCompras.TextMatrix(j, 1) = "pagoLocExt" Then
                        ...
                        Nodo3.appendChild Nodo4
                    End If
                    If vsfgDescCompras.TextMatrix(j, 1) = "formaPago" And Trim(vsfgCompras.TextMatrix(i, j)) <> "" Then
