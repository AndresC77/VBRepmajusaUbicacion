...
Begin VB.Form frmCatCliente 
   BackColor       =   &H00DDDDDD&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Categoria Cliente"
   ClientHeight    =   6960
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8280
   Icon            =   "frmCatCliente.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmCatCliente.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmCatCliente"
...
Private Sub IniDato()
    Tipo = "Categoría de Cliente"
    Tipo2 = "el Categoría de Cliente"
    Me.Caption = Tipo
End Sub
...
End Sub
Private Sub Carga()
    strSql = " SELECT cat_p_codigo,cat_p_nombre,lis_pre_codigo,cat_p_ctaconta,cat_p_ctaconta_ant,cat_p_fechamod, cat_p_usumod, '0' as modi " & _
             " FROM categoria_p  " & _
             " WHERE cat_p_tipo='C' " & _
             " AND emp_codigo='" & strEmpresa & "'"
     If chkFiltroCodigo.value = 1 Then
        strSql = strSql & " AND cat_p_codigo LIKE  '%" & txtCodigo.Text & "%' "
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND cat_p_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY cat_p_nombre "
    ...
    'crea combo
    strSql = " SELECT lis_pre_codigo, CONCAT(lis_pre_codigo,' - ',lis_pre_descripcion) as lis_pre_nombre" & _
            " FROM lista_precio " & _
            " WHERE emp_codigo = '" & strEmpresa & "'" & _
            " ORDER BY lis_pre_codigo"
    clsCon_Def.Ejecutar strSql
    VSFG.ColComboList(3) = VSFG.BuildComboList(clsCon_Def.adorec_Def, " *lis_pre_nombre", "lis_pre_codigo")
    ucrtVSFG.PonerNum
End Sub
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            strSql = " UPDATE categoria_p " & _
                     " SET cat_p_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                     " lis_pre_codigo='" & VSFG.TextMatrix(i, 3) & "'," & _
                     " cat_p_ctaconta='" & UCase(VSFG.TextMatrix(i, 4)) & "'," & _
                     " cat_p_ctaconta_ant='" & UCase(VSFG.TextMatrix(i, 5)) & "'," & _
                     " cat_p_fechamod=CURRENT_TIMESTAMP," & _
                     " cat_p_usumod='" & strUsuario & "' " & _
                     " WHERE cat_p_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta categoría", vbInformation, "Ingreso"
                control = 1
            ElseIf VSFG.TextMatrix(i, 3) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta la lista de precios", vbInformation, "Ingreso"
                control = 1
            ...
            ElseIf VSFG.TextMatrix(i, 5) = "" Then
                ...
                control = 1
            Else
                strSql = " SELECT cat_p_codigo" & _
                    " FROM categoria_p " & _
                    " WHERE emp_codigo='" & strEmpresa & "' " & _
                    " AND cat_p_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
                    " AND cat_p_tipo='C'"
                clsCon_Def.Ejecutar strSql
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    strSql = " INSERT INTO categoria_p(emp_codigo,cat_p_codigo," & _
                        " cat_p_nombre,cat_p_tipo,cat_p_ctaconta,cat_p_ctaconta_ant,lis_pre_codigo,cat_p_fechamod,cat_p_usumod) " & _
                        " VALUES ('" & strEmpresa & "','" & UCase(VSFG.TextMatrix(i, 1)) & "','" & UCase(VSFG.TextMatrix(i, 2)) & "','C', " & _
                        " '" & UCase(VSFG.TextMatrix(i, 4)) & "','" & UCase(VSFG.TextMatrix(i, 5)) & "','" & VSFG.TextMatrix(i, 3) & "',CURRENT_TIMESTAMP, '" & strUsuario & "')"
                    clsCon_Def.Ejecutar strSql, "M"
                Else
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            ...
            strSql = " SELECT count(per_codigo) as existe " & _
                    " FROM persona " & _
                    " WHERE per_tipo='C' " & _
                    " AND emp_codigo='" & strEmpresa & "'" & _
                    " AND cat_p_codigo='" & VSFG.TextMatrix(i, 1) & "'"
            clsCon_Def.Ejecutar (strSql)
