...
Begin VB.Form frmTipoNegocio 
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmTipoNegocio.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmTipoNegocio"
...
End Sub
Private Sub Carga()
    strSql = " SELECT tip_ped_codigo,tip_ped_nombre,tip_ped_ptofac,tip_ped_item,tip_ped_fechamod,tip_ped_usumod, '0' as modi " & _
             " FROM tipo_pedido " & _
             " WHERE emp_codigo = '" & strEmpresa & "'"
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  tip_ped_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  tip_ped_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
           strSql = " UPDATE tipo_pedido " & _
                 " SET tip_ped_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                 " tip_ped_ptofac='" & VSFG.TextMatrix(i, 3) & "'," & _
                 " tip_ped_item='" & FormatoD0(VSFG.TextMatrix(i, 4)) & "'," & _
                 " tip_ped_fechamod=CURRENT_TIMESTAMP," & _
                 " tip_ped_usumod='" & strUsuario & "' " & _
                 " WHERE tip_ped_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 3) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta el punto de facturación", vbInformation, "Ingreso"
                control = 1
            Else
                strSql = " SELECT tip_ped_codigo" & _
                        " FROM tipo_pedido " & _
                        " WHERE tip_ped_codigo='" & UCase(VSFG.TextMatrix(i, 1)) & "'" & _
                        " AND emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar strSql
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    strSql = " INSERT INTO tipo_pedido(tip_ped_codigo,emp_codigo,tip_ped_nombre,tip_ped_ptofac,tip_ped_item,tip_ped_fechamod,tip_ped_usumod) " & _
                            " VALUES ('" & UCase(VSFG.TextMatrix(i, 1)) & "','" & strEmpresa & "','" & UCase(VSFG.TextMatrix(i, 2)) & "','" & VSFG.TextMatrix(i, 3) & "','" & FormatoD0(VSFG.TextMatrix(i, 4)) & "', " & _
...
Private Sub VSFG_AfterEdit(ByVal Row As Long, ByVal Col As Long)
    ...
    End If
    If Col = 4 Then
        If Trim(VSFG.TextMatrix(Row, 4)) <> "" And Not IsNumeric(VSFG.TextMatrix(Row, 4)) Then
            MsgBox "Ingrese un número válido", vbInformation, "Núm. Items x Página"
