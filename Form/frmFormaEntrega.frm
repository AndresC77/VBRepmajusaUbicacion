...
Begin VB.Form frmFormaEntrega 
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmFormaEntrega.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmFormaEntrega"
...
Private Sub IniDato()
    Tipo = "Formas de Entrega"
    Tipo2 = "Formas de Entrega"
    Me.Caption = Tipo
End Sub
...
End Sub
Private Sub Carga()
   strSQL = " SELECT for_ent_codigo,for_ent_nombre,for_ent_descripcion,cou_codigo,for_ent_fechamod,for_ent_usumod, '0' as modi  " & _
            " FROM forma_entrega " & _
            " WHERE emp_codigo='" & strEmpresa & "' "
    If chkFiltroCodigo.Value = 1 Then
        strSQL = strSQL & "AND for_ent_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.Value = 1 Then
        strSQL = strSQL & " AND for_ent_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSQL = strSQL & " ORDER BY for_ent_nombre "
    ...
    strSQL = " SELECT cou_codigo,cou_nombre" & _
             ...
             " ORDER BY cou_nombre "
    clsCon_Def.Ejecutar strSQL
    VSFG.ColComboList(4) = VSFG.BuildComboList(clsCon_Def.adorec_Def, "cou_codigo,*cou_nombre", "cou_codigo")
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
           strSQL = " UPDATE forma_entrega " & _
                    " SET for_ent_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                    " for_ent_descripcion='" & UCase(VSFG.TextMatrix(i, 3)) & " '," & _
                    " cou_codigo='" & UCase(VSFG.TextMatrix(i, 4)) & " '," & _
                    " for_ent_fechamod=CURRENT_TIMESTAMP,for_ent_usumod='" & strUsuario & "' " & _
                    " WHERE for_ent_codigo='" & VSFG.TextMatrix(i, 1) & "' AND emp_codigo='" & strEmpresa & "'"
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                ...
                control = 1
            Else
                strSQL = " SELECT for_ent_codigo" & _
                    " FROM forma_entrega " & _
                    " WHERE for_ent_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                    " AND emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar strSQL
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                   strSQL = " INSERT INTO forma_entrega(for_ent_codigo,emp_codigo,for_ent_nombre,for_ent_descripcion,cou_codigo,for_ent_fechamod,for_ent_usumod) " & _
                            " VALUES ('" & UCase(VSFG.TextMatrix(i, 1)) & "','" & strEmpresa & "','" & UCase(VSFG.TextMatrix(i, 2)) & "','" & UCase(VSFG.TextMatrix(i, 3)) & "','" & UCase(VSFG.TextMatrix(i, 4)) & "', " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            strSQL = " SELECT count(for_ent_codigo) As existe " & _
                " FROM persona " & _
                " WHERE for_ent_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                " AND emp_codigo='" & strEmpresa & "'"
            clsCon_Def.Ejecutar (strSQL)
