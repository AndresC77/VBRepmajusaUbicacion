...
End Sub
Private Sub Carga()
    strSql = " SELECT zon_codigo,zon_nombre,zon_fechamod, zon_usumod, '0' as modi " & _
             " FROM zona " & _
             " WHERE zon_codigo LIKE '%'"
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  zon_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  zon_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY zon_nombre "
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            strSql = " UPDATE zona " & _
                 " SET zon_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                 " zon_fechamod=CURRENT_TIMESTAMP," & _
                 " zon_usumod='" & strUsuario & "' " & _
                 " WHERE zon_codigo='" & VSFG.TextMatrix(i, 1) & "'"
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            ...
            strSql = " SELECT count(per_codigo) as existe " & _
                    " FROM persona " & _
                    " WHERE zon_codigo = '" & VSFG.TextMatrix(i, 1) & "' " & _
