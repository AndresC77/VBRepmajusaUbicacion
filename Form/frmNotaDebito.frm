...
Begin VB.Form frmNotaDebito 
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmNotaDebito.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmNotaDebito"
...
Private Sub Carga()
     strSql = " SELECT tip_not_codigo,tip_not_nombre,tip_not_descripcion, " & _
              " tip_not_fechamod, tip_not_usumod, '0' as modi" & _
              " FROM tipo_nota " & _
              " WHERE tip_not_d_c = 'D'"
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  tip_not_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  tip_not_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY tip_not_nombre "
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            strSql = " UPDATE tipo_nota " & _
                     " SET tip_not_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                     " tip_not_descripcion='" & UCase(VSFG.TextMatrix(i, 3)) & "'," & _
                     " tip_not_fechamod=CURRENT_TIMESTAMP," & _
                     " tip_not_usumod='" & strUsuario & "' " & _
                     " WHERE tip_not_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
                     " AND tip_not_d_c='D'"
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                ...
                control = 1
            Else
                 strSql = " SELECT (tip_not_codigo) " & _
                          " FROM tipo_nota " & _
                          " WHERE tip_not_codigo = '" & VSFG.TextMatrix(i, 1) & "'"
                 clsCon_Def.Ejecutar strSql
                'controla que no se repita el código
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    strSql = " INSERT INTO tipo_nota" & _
                             "(tip_not_codigo,tip_not_d_c,tip_not_nombre,tip_not_descripcion,tip_not_fechamod,tip_not_usumod) " & _
                             " VALUES ('" & UCase(VSFG.TextMatrix(i, 1)) & "','D','" & UCase(VSFG.TextMatrix(i, 2)) & "','" & UCase(VSFG.TextMatrix(i, 3)) & "', " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 1 Then
            strSql = " SELECT count(tip_not_codigo) As existe " & _
                     " FROM nota_d_c " & _
                     " WHERE tip_not_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
