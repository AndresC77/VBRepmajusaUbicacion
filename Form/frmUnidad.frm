...
Begin VB.Form frmUnidad 
   ...
   Begin VSFlex8Ctl.VSFlexGrid VSFG 
      ...
      FormatString    =   $"frmUnidad.frx":030A
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
End Sub
Private Sub Carga()
     strSql = " SELECT uni_codigo,uni_nombre,uni_fechamod, uni_usumod, '0' as modi " & _
             " FROM unidad " & _
             " WHERE emp_codigo='" & strEmpresa & "' "
             
  
    If chkFiltroCodigo.value = 1 Then
        strSql = strSql & "AND  uni_codigo LIKE  '%" & txtCodigo.Text & "%'"
    End If
    If chkFiltroNombre.value = 1 Then
        strSql = strSql & " AND  uni_nombre LIKE '%" & txtNombre.Text & "%' "
    End If
    strSql = strSql & " ORDER BY uni_nombre "
...
Private Sub cmbAceptar_Click()
    ...
    For i = 1 To VSFG.Rows - 1
        ...
        If VSFG.TextMatrix(i, VSFG.Cols - 1) = 3 Then
            strSql = " UPDATE unidad " & _
                 " SET uni_nombre='" & UCase(VSFG.TextMatrix(i, 2)) & "'," & _
                 " uni_fechamod=CURRENT_TIMESTAMP," & _
                 " uni_usumod='" & strUsuario & "' " & _
                 " WHERE uni_codigo='" & VSFG.TextMatrix(i, 1) & "' " & _
        ...
        ElseIf VSFG.TextMatrix(i, VSFG.Cols - 1) = 2 Then
            ...
            ElseIf VSFG.TextMatrix(i, 2) = "" Then
                MsgBox "No puede ingresar " & Tipo2 & " falta unidad de medida", vbInformation, "Ingreso"
                control = 1
            Else
                strSql = " SELECT uni_codigo" & _
                    " FROM unidad " & _
                    " WHERE uni_codigo='" & VSFG.TextMatrix(i, 1) & "'" & _
                    " AND emp_codigo='" & strEmpresa & "'"
                clsCon_Def.Ejecutar strSql
                ...
                If clsCon_Def.adorec_Def.RecordCount = 0 Then
                    strSql = " INSERT INTO unidad(uni_codigo,emp_codigo,uni_nombre,uni_fechamod,uni_usumod) " & _
